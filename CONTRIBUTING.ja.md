...
コード内ではエムダッシュ (`—`, U+2014) を使いません。代わりにコロン、カンマ、丸括弧、スペース付きハイフン (`" - "`)、または文の言い換えを使ってください。これはローカルの ESLint ルール `local/no-em-dash` で強制しており、全パッケージのコメントおよび文字列・テンプレートリテラル内のエムダッシュを検出します。

唯一の例外は CLI ランタイムのユーザー向けメッセージ (端末に出力される文字列で、コロンよりエムダッシュの方が自然に読める箇所) です。該当行は明示的にオプトアウトしてください:
...
このルールの対象は **新規** のエムダッシュに限られます。ルール導入前から存在するものは、パッケージごとの `eslint-suppressions.json` にベースラインとして記録されており (ESLint はこのファイルを各パッケージのディレクトリ基準で解決するため、ベースラインはリポジトリのルートではなく各パッケージに置かれます)、CI を失敗させません。すでにベースライン済みのエムダッシュを持つファイルに新しいエムダッシュを追加すると、追加分を取り除くまでそのファイル内の全エムダッシュが報告されます。追加を削除すれば件数がベースラインに戻り、lint は再び通ります。ベースライン済みのエムダッシュを削除しても lint が壊れることはありません。古くなったエントリを後から削除するには、該当パッケージで次を実行してください:
