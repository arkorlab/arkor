...
- [x] **Try it without an account.** `arkor dev` boots straight into a fresh anonymous workspace. Anonymous workspaces are single-device: they live on the machine that issued them. Run `arkor login --oauth` to start the Arkor Cloud OAuth (PKCE) flow and attach the work to an account that follows you across devices.
