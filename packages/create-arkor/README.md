...
`src/arkor/`, `arkor.config.ts`, `README.md`, `.gitignore`, and `package.json` are always written. `AGENTS.md` and `CLAUDE.md` are written by default and skipped with `--no-agents-md`. The two yaml files are conditional:
