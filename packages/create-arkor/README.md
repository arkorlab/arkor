...
- **`.yarnrc.yml`** lands on `--use-yarn` and on fresh scaffolds where package-manager detection didn't resolve a pm (no `--use-<pm>` flag AND `npm_config_user_agent` failed to identify the invoking pm) — pinning `nodeLinker: node-modules` so a yarn-berry user reading the manual-install hint and running `yarn install` doesn't hit the PnP default (the arkor runtime can't resolve modules through PnP). Skipped for explicit non-yarn pms (including the common `npm create arkor` flow, where UA detection lands on npm) and for merges into existing non-empty projects (we don't flip the install mode of an enclosing yarn-berry workspace).
