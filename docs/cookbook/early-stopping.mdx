...
Early stopping is the pattern of cutting a run short once its metrics stop improving or start degrading, instead of training to the end. Arkor does not have it built in; it does have everything you need to bolt it on in a few lines of TypeScript.
...
- **Do not throw inside the callback.** A throw rejects `wait()` immediately; it is not routed through the SSE reconnect loop and not retried (that loop only handles transport failures), so throwing to stop the run just aborts `wait()` abruptly with your error instead of stopping cleanly (see [SDK § Lifecycle callbacks](/sdk/callbacks)). Use the controller; that is the deterministic path.
