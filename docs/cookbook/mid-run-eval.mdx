...
**Forward each checkpoint's sample to a review queue.** The callback's `infer` is permanently bound to the just-saved checkpoint and `InferArgs` has no base-model override, so this callback alone cannot produce a base-vs-adapter comparison. The base-model half needs a separate request through the raw client: construct one the way [`CloudApiClient`](/sdk/deployments) shows, then call `client.chat({ scope: { orgSlug, projectSlug }, body: { messages, baseModel } })`. `chat` is an instance method these docs do not otherwise cover. Studio's Playground Base / Adapter toggle does the same comparison by hand. What the callback can automate by itself is capturing one sample per checkpoint for later review:
...
**Extract typed fields with a JSON schema.** Replace the freeform parse with `responseFormat: { type: "json_schema", json_schema: { name, schema, strict: true } }`. `data.choices[0].message.content` becomes a JSON string that always satisfies your schema, so a single `JSON.parse` gives you a typed object you can branch on directly (`parsed.urgency === "high"`, `parsed.category in VALID_LABELS`). Full pattern, including how to wire the typed fields into early stopping, in the [Structured outputs and function calling recipe](/cookbook/structured-outputs).
...
- **Wrap in `try / catch`.** A throw out of `onCheckpoint` rejects `wait()` immediately; it is not caught by the SSE reconnect loop or retried (that loop only handles transport failures) (see [SDK § Lifecycle callbacks](/sdk/callbacks)). For non-fatal handling, catch the error inside the callback and decide what to do.
