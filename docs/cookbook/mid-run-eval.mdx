...
**Forward each checkpoint's sample to a review queue.** The callback's `infer` is permanently bound to the just-saved checkpoint and `InferArgs` has no base-model override, so this callback alone cannot produce a base-vs-adapter comparison (for the base-model side, issue a separate request with `CloudApiClient.chat({ scope, body: { messages, baseModel } })`, or use Studio's Playground Base / Adapter toggle by hand). What the callback can automate by itself is capturing one sample per checkpoint for later review:
