...
**Extract typed fields with a JSON schema.** Replace the freeform `data.content` parse with `responseFormat: { type: "json_schema", strict: true, json_schema: {...} }` and the body becomes a guaranteed-shape object you can branch on directly (`parsed.urgency === "high"`, `parsed.category in VALID_LABELS`). Full pattern, including how to wire the typed fields into early stopping, in the [Structured outputs and function calling recipe](/cookbook/structured-outputs).
