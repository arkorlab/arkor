...
`infer({ responseFormat })` gets you a hard guarantee in its `json_schema` mode with `strict: true`. The model is constrained at decode time to emit a string that matches the JSON Schema you hand it, so `JSON.parse` succeeds and the resulting object has the keys you asked for. Leave `strict` off and the schema is only a hint; the model can still drift. The weaker `json_object` mode promises parseable JSON and nothing about your keys. The one exception is truncation: a response cut off by `maxTokens` can end mid-document, so keep the limit generous for structured outputs. That turns the mid-run check from "log a sample, eyeball it" into "extract typed fields, branch on them."
...
`responseFormat: { type: "json_schema", json_schema: { name, schema, strict: true } }` is the OpenAI-compatible shape (`strict` lives inside `json_schema`, not at the top level). The schema is forwarded to the inference backend. With `strict: true` it is treated as authoritative: decoding is constrained so the response body is guaranteed to satisfy it, and properties not declared are rejected. Without `strict` the backend takes it as a hint instead.
...
- **Wrap in `try / catch`.** A throw inside the callback rejects `trainer.wait()` immediately; it is not routed through the SSE reconnect loop, which handles transport failures only ([SDK § Lifecycle callbacks](/sdk/callbacks)). For non-fatal handling, catch inside the callback and use a controller for state changes (same convention as the other recipes).
