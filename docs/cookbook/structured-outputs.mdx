...
`responseFormat: { type: "json_schema", strict: true }` is the OpenAI-compatible shape. The schema is forwarded to the inference backend, which constrains decoding so the response body is guaranteed to satisfy it. With `strict: true`, the schema is treated as authoritative; properties not declared are rejected.
...
`responseFormat` is the right knob 90% of the time. For the rest there is `structuredOutputs`, vLLM's superset that adds regex matching, fixed choice lists, custom EBNF grammars, and Llama-style `structural_tag` framing. **Exactly one** of `json` / `regex` / `choice` / `grammar` / `json_object` / `structural_tag` must be set; the type encodes that invariant so you cannot accidentally combine two.
...
Other shapes follow the same pattern: `regex: "^[A-Z]{3}-\\d{4}$"` for ticket-id formats, `grammar: "..."` for an EBNF you maintain. Fields are snake_case (`json_object`, `structural_tag`, `disable_any_whitespace`, `whitespace_pattern`) to match vLLM's wire format exactly.
