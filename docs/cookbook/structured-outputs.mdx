...
`infer({ responseFormat })` gets you a hard guarantee in its `json_schema` mode. The model is constrained at decode time to emit a string that matches the JSON Schema you hand it, so `JSON.parse` succeeds and the resulting object has the keys you asked for; the weaker `json_object` mode only promises parseable JSON, not your keys. The one exception is truncation: a response cut off by `maxTokens` can end mid-document, so keep the limit generous for structured outputs. That turns the mid-run check from "log a sample, eyeball it" into "extract typed fields, branch on them."
