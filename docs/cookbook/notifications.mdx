...
If the webhook request throws (Slack outage, DNS hiccup, a non-2xx response that your code rethrows on), the callback rejects, and an uncaught throw from a callback rejects `trainer.wait()` immediately with that error ([SDK § Lifecycle callbacks](/sdk/callbacks)). It is not routed through the SSE reconnect loop and it is not retried under `maxReconnectAttempts`. So a flaky webhook firing from `onCompleted` would abort the whole `wait()`, and a CI or cron orchestrator awaiting it would treat an otherwise-successful run as failed.
...
- **Inner `try / catch` is mandatory.** Notifications are nice to have; an outage in your webhook should never abort an otherwise-successful run's `wait()`.
