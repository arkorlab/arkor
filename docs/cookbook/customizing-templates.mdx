...
To layer early stopping on top, follow [Early stopping](/cookbook/early-stopping) — it shows the `AbortController` + `trainer.cancel()` pair you need so an aborted run does not keep burning GPU on the backend.
