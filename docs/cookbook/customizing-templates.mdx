...
`evalSteps: 25` is wired in by default so the run produces an eval-loss series alongside the training-loss curve — Studio's loss chart picks `evalLoss` up automatically, and the `onLog` callback above appends ` evalLoss=…` to the terminal line on the steps where the trainer evaluates (every 25 steps with `maxSteps: 100`, so four eval points per run). Drop or change `evalSteps` if you don't want eval, and remove the `evalLoss` branch from `onLog` if you only care about training loss.
