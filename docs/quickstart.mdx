...
Prefer to keep everything on your machine? On an Apple Silicon Mac you can run this whole quickstart locally through MLX with `arkor start --local` / `arkor dev --local`; see [Train locally with MLX](/guides/cli/local-training).
