...
This runs `arkor dev`, which boots Studio at `http://localhost:4000`, or the next free port if that one is taken (it tries a bounded range, then exits), so open the URL it prints. Studio is the UI; `arkor dev` itself does not start training and does not watch your trainer files.
...
- **Customize the trainer.** Open `src/arkor/trainer.ts` and tweak `lora.r`, `maxSteps`, or add more callbacks. The Run training page re-polls a few seconds after each response and rebuilds, so a saved edit is picked up on its own; give it that moment before clicking and the run uses your new code.
