...
If you edit `src/arkor/` between runs, the Run training page picks the change up on its own: it polls Studio's manifest every 5 seconds, and each poll rebuilds your trainer.
...
- **Customize the trainer.** Open `src/arkor/trainer.ts` and tweak `lora.r`, `maxSteps`, or add more callbacks. The Run training page polls every 5 seconds and rebuilds, so a saved edit is picked up on its own; give it that moment before clicking and the run uses your new code.
