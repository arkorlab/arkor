...
This runs `arkor dev`, which boots Studio at `http://localhost:4000`, or the next free port if that one is taken, so open the URL it prints. Studio is the UI; `arkor dev` itself does not start training and does not watch your trainer files.
...
- **Customize the trainer.** Open `src/arkor/trainer.ts` and tweak `lora.r`, `maxSteps`, or add more callbacks. The Run training page polls for the change, so the next click runs your edits.
