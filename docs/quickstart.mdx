...
This runs `arkor dev`, which boots Studio at `http://localhost:4000`, or the next free port if that one is taken (it tries a bounded range of ports and exits if none are free), so open the URL it prints. Studio is the UI; `arkor dev` itself does not start training and does not watch your trainer files.
...
If you edit `src/arkor/` between runs, the Run training page picks the change up on its own: it polls Studio's manifest every few seconds, and each poll rebuilds your trainer.
...
- **Customize the trainer.** Open `src/arkor/trainer.ts` and tweak `lora.r`, `maxSteps`, or add more callbacks. The Run training page polls every few seconds and rebuilds, so a saved edit is picked up on its own; give it that moment before clicking and the run uses your new code.
