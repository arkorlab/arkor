...
<CardGroup cols={2}>
  ...
  <Card title="Train on a local GPU" icon="microchip" href="/cli/local-training">
    <div id="local-gpu-training" />
    Shipped for Apple Silicon: `arkor start --local` and `arkor dev --local` train (and chat) on your own machine via MLX and the `@arkor/local` package. Next up: NVIDIA (CUDA) and ROCm backends for Linux / Windows through the same backend interface.
  </Card>
