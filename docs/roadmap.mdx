...
<CardGroup cols={2}>
  ...
  <Card title="Broader base model support" icon="cubes" href="/sdk/create-trainer">
    ...
    Expand support beyond Gemma 4 to additional open-weight model families, so the `model` field becomes a real menu instead of a single supported value. We're concentrating on Gemma 4 in this earliest stage so we can deliver fast, stable inference and training to as many people as possible.
  </Card>
