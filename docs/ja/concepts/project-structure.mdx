...
- `arkor dev` は Studio の Web サーバーを既定では `127.0.0.1:4000` で起動し、`~/.arkor/studio-token` を書きます。`-p` を明示しない場合、使用中のポートは次の空きポートにフォールバックするので、CLI が表示した URL を開いてください。`src/arkor/` をファイル監視はしません。Studio の `/api/manifest` エンドポイントは `runBuild` を呼び、bundle の content-addressed コピー経由で再 import します（無編集のリビルドは Node のモジュールキャッシュを再利用し、編集はキャッシュをバストします）。Run training ページはこれを 5 秒ごとにポーリングするので、`src/arkor/` の編集はページをリロードしなくても反映されます。`arkor dev` は単独で `.arkor/state.json` を書きません。
