...
- **`.arkor/agent/session-<pid>-<uuid>.json`**（一時的、エージェントモード）。`arkor dev --agent` が書き込みます（ディレクトリー `0700`、ファイル `0600`）。セッションごとの `{ token, url, port, pid }` JSON で、コーディングエージェントがブラウザーなしでローカル Studio API を操作できるようにします。終了時に削除されます。クラッシュすると残ることがあります（無害で削除して安全）。[`arkor dev`](/ja/cli/dev) の「エージェントモード」節を参照。
