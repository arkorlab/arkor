...
dev ループのメモ: Studio の `/api/manifest` エンドポイントはリクエストごとにトレーナーをリビルドし、bundle の content-addressed コピー（`index.<hash>.mjs`、`packages/arkor/src/studio/manifest.ts` を参照）経由で import します。無編集のリビルドは Node のモジュールキャッシュを再利用し、編集があれば別のモジュールになります。Run training ページはこのエンドポイントを数秒ごとにポーリングするので、`src/arkor/` の編集はページをリロードしなくても反映され、セッション中に追加したトレーナーもそのまま Run training ボタンが有効になります。

## Studio が動く場所

`arkor dev` を起動すると CLI は次を行います:

1. Hono サーバーを既定では `127.0.0.1:4000` で起動（ポートは `-p` で変更可能）。`-p` を渡さない場合、使用中のポートは限られた範囲で次の空きポートにフォールバックするので、開くべき URL は CLI が表示したものです。リトライの規則は [`arkor dev`](/ja/cli/dev) を参照。
2. 同一オリジンで Vite + React の SPA を提供し、UI はループバックの `/api/*` 経由で CLI と話します。
3. 起動ごとに CSRF トークンを発行し（`~/.arkor/studio-token` にモード `0600` で保存）、`/api/*` へのすべてのリクエストにこのトークンを要求します。SPA 自体にはトークン検査がありません。ページが最初にトークンを受け取れるのはそのためです。

サーバーはループバックにのみバインドし、ループバック以外の `Host` ヘッダーのリクエストは拒否します。公開 URL もリモートからの到達経路もなく、トークンは `arkor dev` のたびにローテートされます。
...
Studio は開発ツールです。ローカルで動き、ループバックのみを受け付け、`arkor dev` が立ち上がっている間だけ存在します。ファインチューン済みモデルをプロダクションで使う際は、[デプロイメント](/ja/sdk/deployments)として公開し、そのエンドポイントの OpenAI 互換 API をアプリケーションから呼んでください。`infer` は学習中の `onCheckpoint` にだけ渡される関数で、Studio をユーザーに見せるものでもありません。
