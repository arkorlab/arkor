...
サーバーはループバックにのみバインドし、ループバック以外の `Host` ヘッダーのリクエストは拒否します。公開 URL はなくリモートからの到達経路もありません（ローカルからのリクエストにもトークンが必要です）。トークンは `arkor dev` のたびにローテートされます。
