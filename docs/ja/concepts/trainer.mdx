...
| `model`    | `SupportedModel` | バックエンドが現在受け付けるモデルに型が絞られており、サポートされない値はコンパイルエラーになり、`createTrainer` も例外を投げます。[サポートしているモデル](/ja/models) を参照。 |
...
- **`cancel()`** はバックエンドに学習の停止を依頼します。これはベストエフォートのリクエストです。学習がすでに終端状態（completed、failed、cancelled）にある場合バックエンドはエラーを返すことがあるので、catch する準備をしてください。
...
これを TypeScript で行う目的は、学習に [ライフサイクルコールバック](/ja/concepts/lifecycle)（`onStarted`、`onLog`、`onCheckpoint`、`onCompleted`、`onFailed`）を組み込めることです。次に読むべきコンセプトです。
