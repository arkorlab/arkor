...
- 少なくとも 1 本のパース可能な実データフレームを受信した後（keepalive の `ping` フレームと不正なフレームは進捗として数えません）のクリーンなストリーム EOF は、即時再接続をベース遅延（`initialReconnectDelayMs`、デフォルト 1000 ms）で起こし、失敗回数にはカウントしません。ストリームは `Last-Event-ID` で再開します。
- 接続エラー、またはパース可能なデータフレームを 1 本も受信せずの EOF（keepalive ping やパース不能なゴミだけを送って EOF した場合を含む）は失敗としてカウントされ `handleFailure` を経由します: 指数バックオフは `initialReconnectDelayMs * 2 ** attempt`、各試行の遅延は `maxReconnectDelayMs`（デフォルト 60 000 ms）にクランプ、連続失敗カウントは `maxReconnectAttempts` で上限。これは意図的です: ping やゴミだけを送って EOF するような壊れた中継が、ベース遅延で無限ループしないように失敗として計上します。
- 恒久的な接続エラーはリトライループの例外です: `401` / `403`（認証）、`410`（gone）、`426`（アップグレード必須）のレスポンスは、再接続しても同じ結果になるため `wait()` を即座に reject します。`404` は一過性として扱いリトライします（作成直後のジョブのイベントストリームがまだ可視でない場合があるため）。`408`、`429`、`5xx` も同様にリトライします。
...
- `wait()` は接続ごとにアイドル監視（idle watchdog）も行います: このタイマーは接続を開く前から動き始めるため、最初のフレームを待つ区間も、以降のフレーム間の区間も対象に含みます。タイマーの開始（または直近のリセット）から `idleTimeoutMs`（デフォルト 45 000 ms）以内に何らかのフレーム（実イベントまたは keepalive の `ping`）が届かない場合、その接続はサイレントにストールしたとみなされ abort されます。abort 後は他の一過性障害と同じ `handleFailure` 経路を通ります。これは NAT や LB がコネクションを EOF も RST も送らずにアイドル evict するケースの穴を塞ぐものです: この監視がなければ、`wait()` は Node 自身の内部 fetch タイムアウト（この SDK が制御する契約ではなく、あくまで実装詳細）が発火するまで、そのままハングしてしまいます。この監視はフレーム間のネットワークの沈黙のみを計測します: 何らかのフレームが届いた時点で直ちにクリアされ、そのフレームに対するライフサイクルコールバックの完了後にのみ再武装されるため、`onLog` や `onCheckpoint` の遅いコールバックがこれを誤発火させることはありません。`maxReconnectAttempts` と同様、`idleTimeoutMs` も `TrainerInput` からは設定できません。設定できるのは `createTrainer` の第 2 引数 `context` のみです。

この経路は **あくまで** トランスポート障害のためのものです。ユーザーコールバックの throw はこの経路を通りません。再接続もリトライもせず、即座に `wait()` を reject します（[ライフサイクルコールバック § 例外ハンドリング](/ja/sdk/callbacks#例外ハンドリング) を参照）。throw を再接続経由にすると、既に進んだ失敗イベントの `Last-Event-ID` の先から再開してしまい、エラーを握り潰すことになります。決定的で致命的でないエラーハンドリングが必要なら、`wait()` の reject に頼るのではなくコールバック内で catch してください。
