...
`createTrainer` は `start` / `wait` / `cancel` メソッドを持つ `Trainer` オブジェクトを返します（[トレーナー制御](/ja/sdk/trainer-control) を参照）。1 つの例外を除き、この関数は `TrainerInput` のクライアント側バリデーションを **しません**: TypeScript が型付きフィールドをコンパイル時にチェックし、それ以外は `start()` がジョブを作成するときにクラウド API へ転送されるため、不正な値は SDK ではなくバックエンドがエラーとして報告します。例外は `model` で、生成時に [`SUPPORTED_MODELS`](/ja/models) と照合され、サポートされない値では即座に例外を投げます（`arkor build` / `arkor start` の経路は esbuild でバンドルされ型チェックが走らないため、コンパイル時の絞り込みだけではゲートできません）。
...
| `model` | `SupportedModel` | ベースモデル ID。バックエンドが現在受け付けるモデルに型が絞られており、サポートされない値はコンパイルエラーになるほか、実行時にも `createTrainer` が例外を投げます（[サポートしているモデル](/ja/models) を参照）。 |
