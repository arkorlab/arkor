...
| `gpuTypes` | `string[]`。この run を流すマネージド GPU タイプの slug を優先順に並べたリストで、先頭がプライマリ、残りは順に試されるフェイルオーバー候補です。省略するとバックエンドの既定プールを使います。slug はこの SDK 内の固定リストではなく運用側の GPU カタログ由来なので、未知の slug はコンパイル時ではなくジョブ投入時に弾かれます（`Unknown gpuType slug`）。 |
