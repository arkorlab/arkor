...
**`strict: true` は object スキーマに `additionalProperties: false` を必須にします。** OpenAI の strict モードは、ルートを含む全ての `type: "object"` スキーマで `additionalProperties: false` を明示し、すべてのプロパティを `required` に列挙したときだけ満たされます。これを満たさないと backend が `400 invalid_schema` で拒否します。上の例と [cookbook](/ja/cookbook/structured-outputs) の triage レシピはどちらもこの規約に従っているので、自前のスキーマを書くときはそれをコピーしてください。
