---
title: "infer"
description: "onCheckpoint の中からチェックポイントアダプターに対して推論を走らせる。"
---

# `infer`

`infer` は `CheckpointContext` 上の [`onCheckpoint`](/ja/sdk/callbacks#oncheckpoint-step-adapter-job-infer-artifacts-) に渡される関数です。直前に保存されたチェックポイントアダプターに紐づいた推論リクエストを実行し、生の `Response` を返します。トップレベルの `infer` エクスポートはありません。SDK はコールバックの引数として渡し、呼び出しが正しいジョブ + チェックポイントステップに自動的にスコープされるようにしています。
...
| `temperature` | `number?` | サンプリング温度、0〜2。省略時はバックエンドのデフォルト。 |
| `topP` | `number?` | Nucleus サンプリング、0〜1。省略時はバックエンドのデフォルト。 |
| `maxTokens` | `number?` | 応答トークンの最大値。省略時はバックエンドのデフォルト。 |
| `stream` | `boolean?` | デフォルトは **true**（SSE）。単一 JSON ボディが欲しければ `false`。 |
| `stop` | `string \| string[]?` | ストップシーケンス。各文字列は空不可、最大 16 個。空配列は明示的な no-op として受理されます。 |
| `presencePenalty` | `number?` | -2〜2。 |
| `frequencyPenalty` | `number?` | -2〜2。 |
| `seed` | `number?` | 再現性のためのサンプリングシード。 |
| `logprobs` | `boolean?` | トークンごとの logprob を返す。 |
| `topLogprobs` | `number?` | トークンあたりの候補数、0〜20。0 より大きい値には `logprobs: true` が必要で、組み合わせが不整合ならエンドポイントが `400` を返します。 |
| `logitBias` | `Record<string, number>?` | トークン ID（文字列キー）→ -100〜100 のバイアス。数値でないキーは上流で拒否されます。 |
| `streamOptions` | `{ includeUsage?: boolean }?` | OpenAI の `stream_options`。最後の usage チャンクを受け取るかどうかだけを制御し、計測対象は変わりません。 |
| `enableThinking` | `boolean?` | Gemma 4 の thinking トグル。省略するとモデルの chat template の既定が使われ、Gemma 以外のエンドポイントでも安全です。 |
| `reasoningEffort` | `ReasoningEffort?` | `"none"` / `"minimal"` / `"low"` / `"medium"` / `"high"` / `"xhigh"` / `"max"`。Gemma 4 には段階的な effort がないため `"none"` 以外はすべて「thinking オン」に collapse します。`enableThinking` を明示した場合はそちらが優先されます。 |
...
`responseFormat` は OpenAI 互換で、「JSON が欲しい」用途のほぼすべてに対する推奨選択肢です。`structuredOutputs` は `responseFormat` で表現できない制約のためにとっておいてください。
...
**固定 choice。** 出力を小さな enumerated 集合のいずれかに強制します。余計な前置きを許したくない分類タスクのプロンプトに有用です。
...
- `infer` は `CheckpointContext` 上に **のみ** 存在します。完了済みジョブに対する SDK 側の同等物はありません。その用途にはクラウド API を直接叩くか、学習をもう一度起こしてください。Studio の Playground は最終アダプターとチャットする UI レベルのルートです。
...
- [学習中の評価レシピ](/ja/cookbook/mid-run-eval): エンドツーエンドのパターン
