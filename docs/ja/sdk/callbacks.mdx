...
よくある用途: 自前のパイプライン（PostHog、Datadog）にメトリクスを転送、早期発散の検知、カスタム early-stopping の実装。early-stopping については、[`abortSignal`](/ja/sdk/trainer-control#abortsignal) の abort はあなたのローカル `wait()` を止めるだけだと忘れずに。バックエンドの GPU を実際に止めるには、その後 [`trainer.cancel()`](/ja/sdk/trainer-control#cancel) を呼んでください。

## `onCheckpoint({ step, adapter, job, infer, artifacts })`

ラン進行中にバックエンドでアダプタチェックポイントが保存されたときに発火します。`adapter` は `{ kind: "checkpoint", jobId, step }`。`infer` の詳細は [infer](/ja/sdk/infer) ページにあります。要するにチャット形式のリクエストを取り、生の `Response` を返す関数です。
...
TypeScript でファインチューニングする価値の多くがここに集まります: 学習途中のモデルを取り置いたプロンプトで、フルラン完了前に走らせられます。
...
onCheckpoint: async ({ step, infer }) => {
  try {
    ...
    // ログ／メトリクス／コールバックの外側で trainer.cancel() を呼んで自分でラン失敗を決めるか判断
  }
}
```
