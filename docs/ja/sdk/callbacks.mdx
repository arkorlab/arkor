...
コールバック内で throw しても通常の Promise reject のようには **振る舞いません**。SDK のイベントループは dispatch を try/catch で包み、throw を SSE 再接続ハンドラー（`packages/arkor/src/core/trainer.ts:335-364`、続いて `:307-320` の `handleFailure`）にルートします:
