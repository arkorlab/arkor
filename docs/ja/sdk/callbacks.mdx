...
type TrainerCallbacks = {
  onStarted?:    (ctx: { job: TrainingJob }) => void | Promise<void>;
  onLog?:        (ctx: TrainingLogContext) => void | Promise<void>;
  onCheckpoint?: (ctx: CheckpointContext) => void | Promise<void>;
  onCompleted?:  (ctx: { job: TrainingJob; artifacts?: unknown[] }) => void | Promise<void>;
  onFailed?:     (ctx: { job: TrainingJob; error: string }) => void | Promise<void>;
};
```
...
よくある用途は次のとおりです。自前のパイプライン（PostHog、Datadog）へのメトリックス転送、早期発散の検知、カスタム Early Stopping（指標が悪化したら学習を自動で打ち切るパターン。詳しくは [Early Stopping レシピ](/ja/cookbook/early-stopping) を参照）の実装。Early Stopping については、[`abortSignal`](/ja/sdk/trainer-control#abortsignal) の Abort はあなたのローカル `wait()` を止めるだけだと忘れずに。バックエンドの GPU を実際に止めるには、その後 [`trainer.cancel()`](/ja/sdk/trainer-control#cancel) を呼んでください。
...
`onFailed` は **バックエンド側の** 失敗専用です。あなたの他のコールバック内で投げられた例外は `onFailed` には届きません。何が起きるかは下記の [例外ハンドリング](#例外ハンドリング) を参照してください。
...
2. それ以外で `maxReconnectAttempts` が設定されカウンター超過なら、`wait()` はラップされたエラーで reject。
...
決定的なエラーハンドリングが必要ならコールバック内で catch してください（下の 2 つ目の例を参照）。
...
コールバック内で catch して失敗をローカルに閉じ込め、SSE 再接続を起こさないようにする:
