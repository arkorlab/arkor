...
interface TrainingMetrics {
  finalLoss: number | null;   // 最終ステップの loss
  totalSteps: number | null;  // 実際に実行された勾配ステップ数
  totalTime: number | null;   // 学習の実時間（秒）
}
```
...
よくある用途は次のとおりです。自前のパイプライン（PostHog、Datadog）へのメトリックス転送、早期発散の検知、カスタム Early Stopping（指標が悪化したら学習を自動で打ち切るパターン。詳しくは [Early Stopping レシピ](/ja/cookbook/early-stopping) を参照）の実装。Early Stopping を実装する際は、[`abortSignal`](/ja/sdk/trainer-control#abortsignal) の Abort がローカルの `wait()` を止めるだけである点に注意してください。バックエンドの GPU を実際に止めるには、その後 [`trainer.cancel()`](/ja/sdk/trainer-control#cancel) を呼ぶ必要があります。

### `onCheckpoint({ step, adapter, job, infer, artifacts })`

学習中にバックエンドでアダプターチェックポイントが保存されたときに発火します。`adapter` は `{ kind: "checkpoint", jobId, step }`。`infer` の詳細は [infer](/ja/sdk/infer) ページにあります。要するにチャット形式のリクエストを取り、生の `Response` を返す関数です。
...
`metrics` は終了イベントに付随してバックエンドが報告する run 全体のサマリーです。常にオブジェクトなので null ガードなしで分割代入できます。バックエンドが報告しなかった項目は個別に `null` になります。
...
バックエンドからの失敗報告で 1 回発火。`error` は `string`（バックエンドが送ったメッセージ）であり、`Error` インスタンスではありません。`step` は run が落ちた学習ステップで、学習ループに入る前に失敗した場合（データセットのロード失敗、モデルロード時の OOM など）は `null` です。
...
`onFailed` は **バックエンド側の** 失敗専用です。他のコールバック内で投げられた例外は `onFailed` には届きません。何が起きるかは下記の [例外ハンドリング](#例外ハンドリング) を参照してください。
...
- [`createTrainer`](/ja/sdk/create-trainer): これらのコールバックを取り付ける関数
- [実行ライフサイクル](/ja/concepts/lifecycle): 概念的な流れ
- [`infer`](/ja/sdk/infer): `onCheckpoint` に渡される関数
- [トレーナー制御](/ja/sdk/trainer-control): `abortSignal` と `cancel()`
- [Early Stopping レシピ](/ja/cookbook/early-stopping)
