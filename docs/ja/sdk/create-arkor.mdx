...
- `deploy` と `eval` スロット。`ArkorInput` と `Arkor` ではコメントアウトされた予約フィールドとして現れます。今日それらを渡すと型エラーになり、ランタイムパスもありません。前方互換 API ではなく[ロードマップマーカー](/ja/roadmap#deploy-and-eval-slots)として扱ってください。
