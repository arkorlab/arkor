...
// この SDK から設定できるのはこの部分集合。`model_menu` は読み取り専用。
type WritableDeploymentTarget = Exclude<DeploymentTarget, { kind: "model_menu" }>;
```

`model_menu` のデプロイは、公開カタログのメニューに載っているモデルすべてを配信し、呼び出し側がリクエストごとに OpenAI の `model` ボディフィールドで選びます（省略時はカタログのデフォルトモデル）。メニューはリクエスト時に解決されるので、運用側がカタログを変更してもデプロイを再ターゲットする必要はありません。

メニューのデプロイは **この SDK からではなく Web ダッシュボードから作成します**。`createDeployment` と `updateDeployment` は `WritableDeploymentTarget` を取るので、そこに `{ kind: "model_menu" }` を渡すとコンパイルエラーになります。読み取りには制限がなく、`listDeployments` や `getDeployment` は他と同じようにメニューのデプロイを返すため、`target` で分岐するときは 3 つの `kind` すべてを扱ってください。
...
`DeploymentDto` には `expiresAt` もあります。スイーパーがデプロイを削除する ISO タイムスタンプで、無期限の場合は `null` です。値が入るのは匿名のワンクリックデプロイだけです。

型のみの export（`import type` で import できるもの）: `CloudApiClientOptions`、`DeploymentTarget`、`WritableDeploymentTarget`、`DeploymentAuthMode`、`DeploymentRunRetentionMode`、`DeploymentDto`、`DeploymentKeyDto`、`DeploymentScope`、`CreateDeploymentInput`、`UpdateDeploymentInput`、`CreateDeploymentKeyInput`、`CreateDeploymentKeyResult`。
