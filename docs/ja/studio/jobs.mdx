...
- **Cancel run。** ジョブ詳細ページには、ジョブが `queued` / `running` の間 **Cancel run** ボタンが表示されます。バックエンドのジョブをキャンセルする ([`trainer.cancel()`](/ja/sdk/trainer-control#cancel) と同じ呼び出し) ので、学習プロセスが実際に停止します。Run training カードの **Stop training** ボタンはこれとは別物で、ランナーの出力ストリームから離れるだけです。
