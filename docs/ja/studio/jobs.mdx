...
| `training.log` | `step`、`loss`、そして（含まれていれば）`evalLoss` が Loss チャートのデータ配列に追加。生行 `[log] step=<n> loss=<value> evalLoss=<value>` がイベントログに加わる。`evalLoss` を伴わないステップでは `evalLoss=` の部分は省略される。 |
...
Loss チャートは数値 `loss` を含む `training.log` イベントから描画される SVG プロットです。Y 軸は最小値と最大値によるスケーリング、X 軸はステップ番号で、2 つの系列を表示します:

- **Training loss** — 実線のティール色。`training.log` イベントごとに 1 頂点。
- **Eval loss** — 破線のピンク色（点マーカー付き）。数値 `evalLoss` を含むイベントのサブセット（通常は `evalSteps` 刻み）から描画。Eval ポイントが 1 つも来ていない間は凡例にも表示されません。

ホバーすると最寄りステップと両系列の loss 値が出ます。`training.log` イベントが 1 つも来ていなければ、チャート枠は `Waiting for training.log events…`（`training.log` イベント待ち）と読みます。

### 上級モード（Advanced metrics）

チャートヘッダーの **Advanced** トグルを ON にすると、系列ごとの統計パネルが現れます。各カードに表示される項目:

- **Mean CE ± 95% CI** — Loss 値の標本平均と 95% 信頼区間の半幅（Student の t 分布。n > 31 では z = 1.96 にフォールバック）。
