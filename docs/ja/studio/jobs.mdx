...
実行が 2,000 点の上限を超え、チャートが一度でも圧縮を行った後は、上記の Training loss 統計は圧縮がスパイクを優先して残す性質上、実行全体の厳密に偏りのないサンプルというより、現在保持されているデータの形を表すものになります。Eval loss の統計は、その系列自体が上限を超えない限り `evalLoss` の点は全て保持されるため、通常は影響を受けません。
