...
| `training.log` | `step`、`loss`、そして（含まれていれば）`evalLoss` が Loss チャートのデータ配列に追加。イベントログ行は `step=<n> loss=<value> evalLoss=<value>`（イベント名は別カラムに表示される）。`evalLoss` を伴わないステップでは `evalLoss=` の部分は省略される。 |
...
ホバーすると最寄りステップと、そのステップに含まれる `loss` / `evalLoss` のうち存在する値が表示されます（eval-only ステップでは `loss` 値は出ず、その逆も同様）。`training.log` イベントが 1 つも来ていなければ、チャート枠は `Waiting for training.log events…`（`training.log` イベント待ち）と読みます。
