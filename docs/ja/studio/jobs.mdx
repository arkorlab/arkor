...
Run training パネルはページ読み込み時に `/api/manifest` を呼び、その後もページを開いている間はレスポンスの 5 秒後ごとに呼び直します。レスポンスはプロジェクトの `createArkor({ trainer })` のサマリで、これをもとにアクションをラベル付けします:
...
トレーナー選択やフラグを渡す入力フォームはありません: Studio は常に `createArkor` で登録されたトレーナーを走らせ、`arkor start` は `.arkor/build/index.mjs` があれば再利用します。ただし `arkor dev` の下では、この成果物は自動で最新に保たれます。Run training ページが Studio のマニフェストを 5 秒ごとにポーリングし、そのたびにリビルドが走るので、`src/arkor/` の編集はポーリング 1 回のうちに取り込まれ、リロードは不要です。具体的なリビルドルールは [CLI § build / start](/ja/cli/build-and-start) を参照。
