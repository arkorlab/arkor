...
**Cancel run。** ジョブが `queued` / `running` の間、ヘッダーに **Cancel run** ボタンが表示されます。バックエンドのジョブをキャンセルする ([`trainer.cancel()`](/ja/sdk/trainer-control#cancel) と同じ呼び出し) ので、学習プロセスが実際に停止します。Run training カードの **Stop training** ボタンはこれとは別物で、ランナーの出力ストリームから離れるだけです。ローカルモードではトレーナーが `arkor dev` プロセスの持ち物なので、ジョブ自体をキャンセルするまで学習は続きます。
...
- **終了済みジョブのキャンセルなし。** **Cancel run** ボタンはジョブが `queued` / `running` の間だけ表示されます。終了済みのジョブには止めるものがありません。
