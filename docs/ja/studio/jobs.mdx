...
Run training パネルは `/api/manifest` をマウント時に 1 度呼び、その後はレスポンスの 5 秒後に次を呼びます。呼び出しのたびにトレーナーをリビルドするので、リビルドが遅ければ間隔は 5 秒より延びます。レスポンスはプロジェクトの `createArkor({ trainer })` のサマリで、これをもとにアクションをラベル付けします:
...
トレーナー選択やフラグを渡す入力フォームはありません: Studio は常に `createArkor` で登録されたトレーナーを走らせ、`arkor start` は `.arkor/build/index.mjs` があれば再利用します。ただし `arkor dev` の下では、この成果物は自動で最新に保たれます。上記のマニフェストポーリングが呼び出しのたびにリビルドを走らせるので、`src/arkor/` の編集は次のポーリングで取り込まれ、リロードは不要です。具体的なリビルドルールは [CLI § build / start](/ja/cli/build-and-start) を参照。
