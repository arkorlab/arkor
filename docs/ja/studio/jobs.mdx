...
| 列 | ソース |
| --- | --- |
| Status | `Job.status`（`queued` / `running` / `completed` / `failed` / `cancelled`）に、Studio 側で 1 つの表示ステートを足したもの。SSE ストリームが接続済み（または `createdAt` が直近 90 秒以内）かつ `queued` のジョブは `Warming up GPU` として表示される。セルは色付け用の CSS クラスを持つ。 |
