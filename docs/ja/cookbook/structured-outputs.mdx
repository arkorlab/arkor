...
`infer({ responseFormat })` の `json_schema` モードを使うと、その形を**ハード制約**にできます。モデルはデコード時点で「渡された JSON Schema に合致する文字列しか出せない」状態に縛られるので、`JSON.parse` は成功し、結果のオブジェクトは指定したキーを持ちます。制約の弱い `json_object` モードが保証するのはパースできる JSON であることだけで、キーまでは保証しません。唯一の例外は切り詰めです。`maxTokens` で途中打ち切りになったレスポンスはドキュメントの途中で終わり得るため、構造化出力では上限を余裕を持って設定してください。これにより中間チェックは「サンプルを見て目で確認」から「型付きフィールドを抽出して条件分岐する」に変わります。
