...
`infer({ responseFormat })` の `json_schema` モードに `strict: true` を付けると、その形を**ハード制約**にできます。モデルはデコード時点で「渡された JSON Schema に合致する文字列しか出せない」状態に縛られるので、`JSON.parse` は成功し、結果のオブジェクトは指定したキーを持ちます。`strict` を付けないとスキーマは「ヒント」扱いでモデルがブレる余地が残り、制約の弱い `json_object` モードが保証するのはパースできる JSON であることだけで、キーまでは保証しません。唯一の例外は切り詰めです。`maxTokens` で途中打ち切りになったレスポンスはドキュメントの途中で終わり得るため、構造化出力では上限を余裕を持って設定してください。これにより中間チェックは「サンプルを見て目で確認」から「型付きフィールドを抽出して条件分岐する」に変わります。
...
`responseFormat: { type: "json_schema", json_schema: { name, schema, strict: true } }` が OpenAI 互換のシェイプです（`strict` はトップレベルではなく `json_schema` の中）。スキーマは推論バックエンドにそのまま転送されます。`strict: true` を付けるとスキーマが正本として扱われ、デコード時にレスポンス全体がそれを満たすよう制約され、`properties` に宣言されていないキーは拒否されて `required` が強制されます。付けない場合、バックエンドはスキーマをヒントとして扱います。
...
外部ツールを呼ぶ必要があるとき (注文の状態を引く、天気を取る、社内 API を叩く) は、`tools` と `toolChoice` を `infer` に渡します。応答は free-form なテキストではなく `tool_calls` を含む形になり、あなたのコードがそのツールを実行し、続けたければ `tool` ロールのメッセージを足して再度 `infer` を呼びます。
...
- **`try / catch` で囲む。** callback 内での throw は `trainer.wait()` を即座に reject します。SSE 再接続ループには乗せません。再接続ループはトランスポート障害のみを扱います ([SDK § ライフサイクルコールバック](/ja/sdk/callbacks))。致命的でない処理は callback 内で catch し、状態変更には他のレシピと同様に AbortController を使います。
