...
終端の `onCompleted` と `onFailed` コールバックから、Slack や Discord などの Webhook にステータスメッセージを送れます。
...
Webhook リクエストが throw すると（Slack 障害、DNS の不調、コードが 200 番台以外を再 throw するなど）、コールバックは reject します。コールバックからの未捕捉の throw は、そのエラーで `trainer.wait()` を即座に reject させます（[SDK § ライフサイクルコールバック](/ja/sdk/callbacks)）。これは SSE 再接続ループには送られず、`maxReconnectAttempts` の下でリトライされることもありません。つまり `onCompleted` から発火した不安定な Webhook が `wait()` 全体を中断させてしまい、それを待っている CI や cron のオーケストレーターは、本来は成功した学習を失敗として扱ってしまいます。
