...
**チェックポイントごとのサンプルをレビューキューへ送る。** コールバックの `infer` は保存直後のチェックポイントに固定されており、`InferArgs` にベースモデルへ切り替える指定もないため、このコールバック単体ではベース vs アダプター比較はできません（ベースモデル側は `CloudApiClient.chat({ scope, body: { messages, baseModel } })` で別リクエストを発行するか、手動なら Studio Playground の Base / Adapter 切替を使ってください）。コールバック単体で自動化できるのは、チェックポイントごとに 1 サンプルを取得して後からレビューできるようにすることです:
