...
**チェックポイントごとのサンプルをレビューキューへ送る。** コールバックの `infer` は保存直後のチェックポイントに固定されており、`InferArgs` にベースモデルへ切り替える指定もないため、ここから自動でベース vs アダプター比較を行うことはできません（手動なら Studio Playground の Base / Adapter 切替を使ってください）。自動化できるのは、チェックポイントごとに 1 サンプルを取得して後からレビューできるようにすることです:
