...
「Early Stopping（早期停止）」は、学習の指標が改善しなくなったり悪化し始めたら、最後まで回さず途中で打ち切るパターンです。Arkor に Early Stopping は組み込まれていませんが、TypeScript 数行で組み立てるための材料はすべて揃っています。
...
- **コールバック内で throw しない。** throw は即座に `wait()` を reject します。SSE 再接続ループには回されず、リトライもされません（このループはトランスポート障害のみを扱います）。そのため throw で学習を止めようとしても、きれいに止まるのではなく `wait()` がエラーで突然 abort するだけです（[SDK § ライフサイクルコールバック](/ja/sdk/callbacks) 参照）。controller を使うこと。これが決定的に動かす唯一の方法です。
