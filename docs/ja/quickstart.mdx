...
すべてを手元のマシンで完結させたい場合、Apple Silicon Mac なら `arkor start --local` / `arkor dev --local` でこのクイックスタート全体を MLX 経由でローカル実行できます。[MLX でローカル学習する](/ja/guides/cli/local-training)を参照してください。
