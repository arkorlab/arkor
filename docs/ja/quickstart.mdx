...
これは `arkor dev` を実行し、Studio を `http://localhost:4000`、そのポートが使用中なら次の空きポートで起動します（試す範囲には上限があり、尽きると終了します）。表示された URL を開いてください。Studio は UI で、`arkor dev` 自体は学習を開始することも、トレーナーファイルを監視することもしません。
...
学習の合間に `src/arkor/` を編集した場合、Run training ページはその変更を自動で拾います。5 秒ごとに Studio のマニフェストをポーリングし、そのたびにトレーナーをリビルドするためです。
...
- **トレーナーをカスタマイズ。** `src/arkor/trainer.ts` を開き、`lora.r` や `maxSteps` を調整したり、コールバックを追加したり。Run training ページはレスポンスの数秒後に再ポーリングしてリビルドするので、保存した編集は自動で取り込まれます。その数秒を待ってからクリックすれば、新しいコードで実行されます。
