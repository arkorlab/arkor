...
これは `arkor dev` を実行し、Studio を `http://localhost:4000`、そのポートが使用中なら次の空きポートで起動します。表示された URL を開いてください。Studio は UI で、`arkor dev` 自体は学習を開始することも、トレーナーファイルを監視することもしません。
...
- **トレーナーをカスタマイズ。** `src/arkor/trainer.ts` を開き、`lora.r` や `maxSteps` を調整したり、コールバックを追加したり。Run training ページが変更をポーリングするので、次のクリックでそのまま反映されます。
