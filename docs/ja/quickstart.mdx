...
学習の合間に `src/arkor/` を編集した場合は、Run training ページをリロード（または `npx arkor build` を実行）してから次のクリックをすると、新しいコードが動きます。
...
`--oauth` を付けるとループバックポート上で Auth0 PKCE フローを走らせ、ローカルの認証情報をアカウントに紐づけます。以降、このマシンからの学習はどのデバイスからも当該アカウントの下に表示されます。フラグなしの `arkor login` は初期選択が `Anonymous` の対話ピッカーを表示するので、デフォルトを受け入れると既にある使い捨てセッションを更新するだけになります。
