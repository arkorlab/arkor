...
学習の合間に `src/arkor/` を編集した場合、Run training ページはその変更を自動で拾います。数秒ごとに Studio のマニフェストをポーリングし、そのたびにトレーナーをリビルドするためです。
...
- **トレーナーをカスタマイズ。** `src/arkor/trainer.ts` を開き、`lora.r` や `maxSteps` を調整したり、コールバックを追加したり。Run training ページは数秒ごとにポーリングしてリビルドするので、保存した編集は自動で取り込まれます。その数秒を待ってからクリックすれば、新しいコードで実行されます。
