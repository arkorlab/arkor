...
- **トレーナーをカスタマイズ。** `src/arkor/trainer.ts` を開き、`lora.r` や `maxSteps` を調整したり、コールバックを追加したり。Run training ページは 5 秒ごとにポーリングしてリビルドするので、保存した編集は自動で取り込まれます。その数秒を待ってからクリックすれば、新しいコードで実行されます。
