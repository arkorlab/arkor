...
5. OAuth フローは PKCE ペアを生成し、クラウド API から提供されたコールバックポートのいずれかでループバック HTTP サーバーを起動して、authorize URL を開き（または表示し）、コールバックを待ちます。コードをトークンに交換する前に state を検証し、不一致は CSRF 防止のため Abort されます。得られた OAuth トークンは `mode: "auth0"` で `~/.arkor/credentials.json` に書かれます。
...
- `--yes` はプロンプトをスキップしますが、匿名認証情報を削除する前の復元不可警告は表示します。
...
両モードとも同じファイル `~/.arkor/credentials.json` に書き、`mode` フィールドが `"auth0"` か `"anon"` でタグ付けされます。これにより CLI（と Studio サーバー）はどのモードを使うかわかります。レイアウト全体は [プロジェクト構成](/ja/concepts/project-structure) を参照。
