...
| `--use-bun` | パッケージマネージャーに bun を強制。（Windows 既知の問題: `arkor init --use-bun` は `node_modules` までは作成されるものの `bun.lock` が生成されないため、bootstrap commit の lockfile-in-HEAD 不変条件が成立しません。この理由で CI install matrix では Windows × bun を `arkor init` lane から外しています。`.github/workflows/ci.yaml` の `install-matrix` セクションコメント参照。`bun install` 直接実行および `create-arkor --use-bun` は Windows で正常動作しており、`arkor init` の spawn shape に固有の divergence です。） |
