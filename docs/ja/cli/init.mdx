...
ツールが触るパスは最大 8 つで、無条件の 6 つに加え、パッケージマネージャー設定 2 つが条件付きで追加されます。いずれもユーザーの既存内容を上書きしません:
