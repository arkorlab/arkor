...
`src/arkor/index.ts`、`src/arkor/trainer.ts`、`arkor.config.ts`、スターター `package.json` を **カレントディレクトリー** に生成します。既存リポジトリに Arkor を追加したいときに使います（`pnpm create arkor my-app` のフローは同じツールを 1 つ上のディレクトリーで実行します）。
