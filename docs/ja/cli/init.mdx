...
scaffold は `package.json` の隣に小さな `pnpm-workspace.yaml` を出力します。
...
— あるいは後から `pnpm-workspace.yaml` のエントリーを `true` に書き換えてください。`--use-<pm>` の指定にかかわらずこのフラグは受け付けます。`--use-npm` で scaffold して後から pnpm に切り替える場合に、過去の `--allow-builds` の指定が静かに失われないようにするためです。
