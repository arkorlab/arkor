---
title: "arkor init"
description: "カレントディレクトリに Arkor プロジェクトをスキャフォールド。"
---
...
| `--template <id>` | スターターテンプレート。表示される選択肢: `triage`、`translate`、`redaction`。バリデータは明示的に渡したときに限り隠し ID（`minimal`、`alpaca`、`chatml`）も受け付けます。未知の ID はファイル操作前に throw します。 |
...
`--git` と `--skip-git` は排他です。両方渡すと throw します。
...
| `redaction` | PII の伏字 | `{ redactedText, redactedCount, tags }` |
...
| メッセージ | 意味 | 対処 |
| --- | --- | --- |
| `Pick one of --git / --skip-git, not both.` | git 関連の両フラグを渡した。 | どちらか一方にする。 |
| `Unknown template "<id>". Available: triage, translate, redaction, minimal, alpaca, chatml` | `--template` の値がどの登録テンプレート ID にも一致しなかった。 | 一覧の ID を使う。隠し ID（`minimal`、`alpaca`、`chatml`）は明示指定時のみ有効。 |
| `Commit signing failed — created an unsigned commit.` | `git init` は成功したが初回コミットを GPG 署名できなかった。 | 署名のセットアップを直したら `git commit --amend -S` で再署名。リポジトリ自体は問題なし。 |
...
| `arkor.config.ts` | 無ければプレースホルダの export として **作成**。あれば **そのまま**。[プロジェクト構成](/ja/concepts/project-structure) を参照。 |
