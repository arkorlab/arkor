...
`arkor` CLI は Arkor プロジェクトを取り巻くコマンド面です。[プロジェクト生成ツール](/ja/quickstart#1-プロジェクトを生成する)が `arkor` をローカル devDependency としてインストールするので、以下のコマンドは生成済みのプロジェクトの中で実行することを想定しています（`arkor` バイナリはあなたのパッケージマネージャー経由で参照できます）。以降のスニペットはタブグループになっており、普段使うパッケージマネージャーを選べばページ全体がそれに揃います。
...
- [SDK 概要](/ja/sdk/overview): CLI の周りのプログラム面
