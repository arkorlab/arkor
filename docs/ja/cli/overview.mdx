...
`arkor` CLI は Arkor プロジェクトを操作するためのコマンドインターフェイスです。[プロジェクト生成ツール](/ja/quickstart#1-プロジェクトを生成する)が `arkor` をローカル devDependency としてインストールするので、以下のコマンドは生成済みのプロジェクト内で実行することを想定しています（`arkor` バイナリーはお使いのパッケージマネージャー経由で参照できます）。以降のスニペットはタブグループになっており、普段使うパッケージマネージャーを選べばページ全体がそれに揃います。
...
| [`arkor dev`](/ja/cli/dev) | [Studio](/ja/concepts/studio) を `http://localhost:4000` で起動（設定可）。`--local` を渡すと Arkor Cloud の代わりに[ローカル学習サーバー](/ja/cli/local-training)に対して動く。 |
| [`arkor build`](/ja/cli/build-and-start) | `src/arkor/index.ts` を `.arkor/build/index.mjs` にバンドル。 |
| [`arkor start`](/ja/cli/build-and-start) | ビルド成果物を実行（無いときは自動ビルド）。`--local` を渡すと MLX 経由で[このマシンー上で学習](/ja/cli/local-training)する。 |
...
| [`arkor logout`](/ja/cli/auth) | 確認後に `~/.arkor/credentials.json` を削除。匿名 ID は削除後に復元できません。 |
| [`arkor whoami`](/ja/cli/auth) | 現在の ID と到達できる org を表示。 |
...
| `ARKOR_LOCAL_SERVER_URL` / `ARKOR_LOCAL_SERVER_TOKEN` | `arkor start --local` / `arkor dev --local` とトレーナーの間のハンドオフ: プロセス内の[ローカル学習サーバー](/ja/cli/local-training)の URL と起動ごとの bearer トークン。CLI が管理するもので（プロセス内で設定され、Studio の Run training 子プロセスにも注入される）、手動で設定する想定ではなく、片方だけ設定するとエラーになる。 |
...
`arkor` パッケージは `runTrainer` を再エクスポートしているので、`arkor start` 経由でなく自分の TypeScript コードから学習を実行できます。
