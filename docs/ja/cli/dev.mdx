...
| ``Port <requested> is already in use, and no free port was found in <requested>-<last>. Pass --port to choose one explicitly.``（ポート `<requested>` は使用中で、`<requested>`-`<last>` の範囲に空きポートが見つかりませんでした） | `--port` を指定しておらず、デフォルトの `4000` から10 回目の試行までのすべてのポートが使用中だった。 | 競合しているプロセスのいずれかを停止するか、`--port <other>` を明示的に指定してください。 |
