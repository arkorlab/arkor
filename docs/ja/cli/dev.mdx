...
| `--local` | オフ | Arkor Cloud の代わりにローカル学習サーバー (別途インストールする `@arkor/local` ランタイム、Apple Silicon 上の MLX) に対して Studio を動かす。資格情報はブートストラップされず、ジョブはこのマシンー上で学習し、Playground はローカルの推論サーバーを通じてチャットする。[ローカル学習](/ja/cli/local-training)を参照。 |
