...
- **`--port` を指定していない場合（デフォルトの `4000` を使用）。** `4000` が既に使用されている場合（別の `arkor dev` が残っている、無関係な dev サーバーなど）、`arkor dev` は `4001`、`4002` と自動的に次のポートを試します（最大10回まで）。実際にバインドできたポートは ``Arkor Studio running on http://localhost:<port>`` として表示されます。範囲内のすべてのポートが使用中の場合は、``Port <requested> is already in use, and no free port was found in <requested>-<last>. Pass --port to choose one explicitly.`` というメッセージとともに非ゼロで終了します。
