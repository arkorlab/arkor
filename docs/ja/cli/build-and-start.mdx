...
`.arkor/build/index.mjs`（`outDir` のデフォルトは `.arkor/build`）。出力は単一の ESM ファイル、Node 22.22 をターゲットにし、`packages: "external"` で bare specifier（`arkor`、`node_modules` 内のもの）はバンドルせず、成果物はあなたの `node_modules` からランタイム SDK を解決します。インライン化されるのは相対 import のみです。
