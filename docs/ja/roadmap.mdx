---
title: "ロードマップ"
description: "Arkor で次に取り組む項目。"
---

Arkor はアルファ段階なので、このページは意図的に最小限の構成です。各項目は、現在実装中・スコープが固まり順番待ち・検討中、のいずれの状態にあるかでセクション分けしています。日付へのコミットはまだしません。

## 進行中

<CardGroup cols={2}>
  <Card title="ホスト型推論エンドポイント URL" icon="plug" href="/ja/sdk/infer">
    <div id="hosted-inference-endpoint" />
    ファインチューニング済みモデルごとに安定した HTTPS エンドポイント URL を提供し、SDK を経由せずプロダクトから直接呼び出せるようにする。
  </Card>

  <Card title="テンプレートなしのプロジェクト作成" icon="file-circle-plus" href="/ja/cli/init">
    <div id="template-free-projects" />
    スターターテンプレートを選ばずにプロジェクトを生成できるようにする。今は `create-arkor` と `arkor init` で `triage` / `translate` / `redaction` のいずれかを必ず選ぶ必要があるが、空の trainer からスタートする道を開く。
  </Card>

  <Card title="ローカル GPU での学習" icon="microchip" href="/ja/cli/local-training">
    <div id="local-gpu-training" />
    Apple Silicon 向けにリリース済み。`arkor start --local` と `arkor dev --local` が、MLX と `@arkor/local` パッケージ経由で自分のマシーン上での学習 (とチャット) を実行する。次は同じバックエンドインターフェースを通じた Linux / Windows 向けの NVIDIA (CUDA) と ROCm バックエンド。
  </Card>
</CardGroup>

## 次に着手

<CardGroup cols={2}>
  <Card title="OAuth トークンの自動リフレッシュ" icon="key" href="/ja/cli/auth">
    <div id="oauth-token-auto-refresh" />
    アクセストークンの期限切れ時に自動でリフレッシュし、長時間のセッションが再ログインで中断されないようにする。
  </Card>

  <Card title="JSONL データセットの持ち込み" icon="file-lines" href="/ja/sdk/dataset">
    <div id="jsonl-dataset" />
    既存の HuggingFace 名と blob URL に加えて、ローカルの JSONL ファイルを学習データセットとしてアップロードできるようにする。
  </Card>

  <Card title="Studio からのドライラン" icon="flask" href="/ja/concepts/studio">
    <div id="studio-dry-run" />
    本番の学習を回す前にスモークテストを打てるよう、既存のドライランオプションを Studio UI に露出する。
  </Card>

  <Card title="Gemma 4 ファミリーへの完全対応" icon="layer-group" href="/ja/sdk/create-trainer">
    <div id="gemma-4-family" />
    `model` フィールドで Gemma 4 ファミリー全体を選べるようにする。現在は、全てのスターターテンプレートが `gemma-4-E4B-it` を使用しているが、レイテンシーと品質のバランスに合わせてサイズを選べる状態に開く。
  </Card>
</CardGroup>

## バックログ

<CardGroup cols={2}>
  <Card title="学習バックエンドのセルフホスト" icon="server" href="/ja/cli/overview">
    <div id="self-hosted-deployments" />
    学習バックエンドを自前のインフラで動かす。`ARKOR_CLOUD_API_URL` のドキュメント化されたノブとバージョン管理された API 保証を含む。
  </Card>

  <Card title="deploy と eval スロット" icon="rocket" href="/ja/sdk/create-arkor">
    <div id="deploy-and-eval-slots" />
    `createArkor` を学習だけでなく、モデルのデプロイと評価まで含む包括的なエントリーポイントへ拡張する。
  </Card>

  <Card title="学習済みモデルのダウンロード" icon="download">
    <div id="download-trained-models" />
    Arkor のマネージド推論に留めずに、自前の環境やデプロイ先で動かせるファイルとして学習済みモデルを書き出す。
  </Card>

  <Card title="シードセットからの合成データ生成" icon="flask-vial">
    <div id="synthetic-data" />
    ラベル付きデータセットがまだ無いケース向けに、小さなシードセットから学習データを合成する。
  </Card>

  <Card title="蒸留テンプレート" icon="link">
    <div id="distillation-templates" />
    互換性のある teacher / student モデルをペアにして、蒸留ジョブがそのまま走るテンプレートを提供する。
  </Card>

  <Card title="オンデバイスモデルのテンプレート" icon="mobile-screen">
    <div id="on-device-templates" />
    WebGPU やモバイルなど、小さめのモデルが必要な環境向けのテンプレートを用意する。
  </Card>

  <Card title="対応ベースモデルの拡充" icon="cubes" href="/ja/sdk/create-trainer">
    <div id="more-base-models" />
    Gemma 4 ファミリー以外のオープンウェイトモデルファミリーへサポートを広げ、`model` フィールドを実質一択ではなく実際に選べるメニューにする。最初期段階の現在は、より多くの人に、より速く安定した推論と学習を提供するため、まずは Gemma 4 ファミリーに集中する。
  </Card>

  <Card title="マルチモーダル学習" icon="images" href="/ja/sdk/create-trainer">
    <div id="multimodal-training" />
    テキストに加えて画像 (将来的には音声) を入力としたファインチューニングに対応する。今はテンプレートも全てテキスト入出力。
  </Card>
</CardGroup>
