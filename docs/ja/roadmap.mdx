...
<CardGroup cols={2}>
  ...
  <Card title="ローカル GPU での学習" icon="microchip" href="/ja/cli/local-training">
    <div id="local-gpu-training" />
    Apple Silicon 向けにリリース済み。`arkor start --local` と `arkor dev --local` が、MLX と `@arkor/local` パッケージ経由で自分のマシンー上での学習 (とチャット) を実行する。次は同じバックエンドインターフェースを通じた Linux / Windows 向けの NVIDIA (CUDA) と ROCm バックエンド。
  </Card>
</CardGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Gemma 4 ファミリーへの完全対応" icon="layer-group" href="/ja/sdk/create-trainer">
    ...
    `model` フィールドで Gemma 4 ファミリー全体を選べるようにする。現在は、全てのスターターテンプレートが `gemma-4-E4B-it` を使用しているが、レイテンシーと品質のバランスに合わせてサイズを選べる状態に開く。
  </Card>
</CardGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="学習済みモデルのダウンロード" icon="download">
    ...
    Arkor のマネージド推論に留めずに、自前の環境やデプロイ先で動かせるファイルとして学習済みモデルを書き出す。
  </Card>
