...
A throw out of `onCheckpoint` rejects `trainer.wait()` immediately; it is not caught by the runtime's reconnect loop, which retries transport failures only. For non-fatal inference errors, catch inside the callback so a recoverable failure doesn't abort the whole run.
