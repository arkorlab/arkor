...
**`strict: true` requires `additionalProperties: false` on every object schema.** OpenAI's strict mode is satisfied only when each `type: "object"` schema (the root, plus every nested object) explicitly sets `additionalProperties: false` and lists every property in `required`. Schemas that omit it are rejected by the backend with a `400 invalid_schema`. The example above and the triage recipe in the [cookbook](/cookbook/structured-outputs) both follow this rule; copy them when you write your own schema.
