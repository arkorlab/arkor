...
| `temperature` | `number?` | Sampling temperature, 0–2. Backend default if omitted. |
| `topP` | `number?` | Nucleus sampling, 0–1. Backend default if omitted. |
...
| `stop` | `string \| string[]?` | Stop sequence(s). Each string must be non-empty, at most 16 entries; an empty array is accepted as an explicit no-op. |
| `presencePenalty` | `number?` | -2–2. |
| `frequencyPenalty` | `number?` | -2–2. |
| `seed` | `number?` | Sampling seed for reproducible completions. |
| `logprobs` | `boolean?` | Return per-token logprobs. |
| `topLogprobs` | `number?` | Alternatives per token, 0–20. Values above 0 require `logprobs: true`; the endpoint returns a `400` for the mismatched pair. |
| `logitBias` | `Record<string, number>?` | Token id (as a string key) → bias in -100–100. Non-numeric keys are rejected upstream. |
| `streamOptions` | `{ includeUsage?: boolean }?` | OpenAI `stream_options`. Controls whether the final usage chunk reaches you; it does not change what is metered. |
| `enableThinking` | `boolean?` | Gemma 4's thinking toggle. Omit to take the model's chat-template default, which is safe on non-Gemma endpoints too. |
| `reasoningEffort` | `ReasoningEffort?` | `"none"` / `"minimal"` / `"low"` / `"medium"` / `"high"` / `"xhigh"` / `"max"`. Gemma 4 has no graduated effort, so every value except `"none"` collapses to "thinking on" there; an explicit `enableThinking` always wins. |
