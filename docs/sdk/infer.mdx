...
interface InferArgs {
  ...
  /** OpenAI-compatible response_format (text / json_object / json_schema / structural_tag). */
...
| `{ type: "structural_tag", format }` *or* `{ type: "structural_tag", structures, triggers }` | Llama-style framed output | Constrain regions wrapped in framing markers (e.g. tool-call frames) to a schema. vLLM 0.20 accepts either the modern `format` shape or the legacy `structures` + `triggers` pair (but not both at once); useful when the output mixes free text with framed structured fragments. |
...
export type ResponseFormat =
  ...
  | {
      // vLLM 0.20 accepts either the modern shape (`format` field
      // alone) or the legacy shape (`structures` + `triggers` pair),
      // not both at once.
      type: "structural_tag";
      format?: unknown;
      structures?: Array<{
...
// `json_object` / `structural_tag` must be set. Field names are
// snake_case to match vLLM's wire format verbatim.
...
- **`responseFormat: { type: "structural_tag" }`** — `choices[0].message.content` is free text interleaved with `begin` / `end`-framed regions whose interiors satisfy the per-structure schema. Parse with the framing markers you supplied.
- **`structuredOutputs: { choice }` / `{ regex }` / `{ grammar }` / `{ structural_tag }`** — `choices[0].message.content` is a string matching the constraint. Not JSON; do not parse.
...
The SDK returns the raw `Response`, so error responses surface as a non-OK status with a JSON body. The codes worth knowing:
...
| `400` schema-validation error | `responseFormat.json_schema.schema` or a `tools[i].function.parameters` is not a valid JSON Schema; `structuredOutputs` was passed with zero or more than one constraint set; `structuredOutputs` carries a constraint *and* `responseFormat` already supplies one (two constraints conflict at vLLM); `structuredOutputs.json_object: false` (only `true` activates JSON-object mode); or a legacy `guided_json` / `guided_regex` / `guided_choice` / `guided_grammar` / `guided_decoding_backend` field was sent (vLLM 0.20 dropped these — migrate to `structuredOutputs`). | Fix the schema / pick exactly one constraint / drop the conflicting field. The TS type rejects multiple `structuredOutputs` keys at compile time; the runtime status is only hit for runtime-built constraint objects, raw HTTP callers, or legacy SDKs. |
| `4xx` model rejection | Backend rejected the request (e.g. context length exceeded, unsupported message shape). | Body carries the upstream error message — surface it to the caller. |
| `5xx` upstream | Inference cluster outage / cold start timeout. | Retryable; the SSE reconnect loop covers most of this for streaming reads. |

The SDK does not retry these for you. Wrap the call in `try / catch` and decide per error class.
