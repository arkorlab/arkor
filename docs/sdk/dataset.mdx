...
| `split` | `string?` | Which split to load. Optional, but the backend fills in the literal `"train"` when you omit it rather than letting the dataset pick — so a dataset that publishes no `train` split needs this set explicitly. |
...
- [`createTrainer`](/sdk/create-trainer) for the `dataset` field that consumes this type
- [Customizing templates recipe](/cookbook/customizing-templates) for swapping the bundled HuggingFace dataset
