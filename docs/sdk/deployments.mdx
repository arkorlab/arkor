...
const client = new CloudApiClient({
  ...
  // anonymous credentials were issued against (so the same client
  // reuses your staging / self-hosted endpoint without any extra
  // config), and finally the production endpoint as a fallback.
  baseUrl: defaultArkorCloudApiUrl(credentials),
  credentials,
});
```

`CloudApiClient` accepts the same `Credentials` shape used everywhere else in the SDK: an OAuth access token (after `arkor login`) or an anonymous CLI token. The API surface is identical for both. The `baseUrl` you hand it must match the control plane the credentials were issued against — passing an OAuth token from staging into a production `CloudApiClient` (or vice versa) will 401 even though the token itself is well-formed.
...
Type-only exports (importable with `import type`): `CloudApiClientOptions`, `DeploymentTarget`, `DeploymentAuthMode`, `DeploymentRunRetentionMode`, `DeploymentDto`, `DeploymentKeyDto`, `DeploymentScope`, `CreateDeploymentInput`, `UpdateDeploymentInput`, `CreateDeploymentKeyInput`, `CreateDeploymentKeyResult`.
