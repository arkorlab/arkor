...
`CloudApiClient` is the typed entry point for managing these deployments programmatically (creating them, rotating API keys, deleting them) from a Node.js script or from your own server. The Studio dashboard covers the common create / toggle / key-management actions, but several operations are SDK-only today (re-targeting an existing deployment, configuring non-default run retention, bulk scripting); see the [Studio Endpoints page](/studio/endpoints) for the current parity boundary. This page covers the SDK surface.
...
// The subset this SDK can set. `model_menu` is read-only here.
type WritableDeploymentTarget = Exclude<DeploymentTarget, { kind: "model_menu" }>;
```

A `model_menu` deployment serves every model published to the public catalog menu, and the caller picks one per request through the OpenAI `model` body field (falling back to the catalog default when it is omitted). The menu resolves at request time, so operator changes to the catalog take effect without re-targeting the deployment.

Menu deployments are **created from the web dashboard, not from this SDK**: `createDeployment` and `updateDeployment` take a `WritableDeploymentTarget`, so `{ kind: "model_menu" }` is a compile error there. Reads are unrestricted: `listDeployments` and `getDeployment` return menu deployments like any other, so handle all three `kind` values when you branch on `target`.
...
Runtime exports (classes, importable as values): `CloudApiClient`, `CloudApiError`.

`DeploymentDto` also carries `expiresAt`: an ISO timestamp after which the deployment is swept, or `null` when it never expires. Only anonymous one-click deployments carry a deadline.

Type-only exports (importable with `import type`): `CloudApiClientOptions`, `DeploymentTarget`, `WritableDeploymentTarget`, `DeploymentAuthMode`, `DeploymentRunRetentionMode`, `DeploymentDto`, `DeploymentKeyDto`, `DeploymentScope`, `CreateDeploymentInput`, `UpdateDeploymentInput`, `CreateDeploymentKeyInput`, `CreateDeploymentKeyResult`.
