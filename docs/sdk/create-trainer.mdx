...
| `gpuTypes` | `[string, ...string[]]`. Ordered list of managed GPU-type slugs: the first entry is the primary, the rest are failover candidates tried in order. Omit to take the backend's default pool; an empty list is a compile error, since it names no primary. The slugs come from the operator's GPU catalog rather than a fixed list in this SDK, so an unrecognised one is rejected at job submission (`Unknown gpuType slug`) instead of at compile time. |
