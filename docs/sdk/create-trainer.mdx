...
`createTrainer` returns a `Trainer` object with `start` / `wait` / `cancel` methods (see [Trainer control](/sdk/trainer-control)). With one exception, the function does **not** run client-side validation on `TrainerInput`: TypeScript checks the typed fields at compile time, and the rest is forwarded to the cloud-api when `start()` creates the job, so bad values are reported by the backend rather than by the SDK. The exception is `model`, which is checked against [`SUPPORTED_MODELS`](/models) at construction and throws immediately on an unsupported value (the `arkor build` / `arkor start` path bundles with esbuild and never typechecks, so the compile-time narrowing alone cannot gate it).
...
The full `TrainerInput` shape is described under Parameters below; the returned `Trainer` is documented on the [Trainer control](/sdk/trainer-control) page. The second `context` argument is annotated `@internal` and is reserved for tests and advanced overrides. `TrainerInternalContext` is not exported from `arkor` because the shape is unstable; its documented reconnect-related fields (`reconnectDelayMs`, `maxReconnectDelayMs`, `maxReconnectAttempts`, `idleTimeoutMs`) are covered under [Trainer control > Reconnects](/sdk/trainer-control#reconnects).
...
| `model` | `SupportedModel` | Base model identifier, narrowed to the models the backend accepts today. An unsupported value is a compile error, and `createTrainer` throws on it at run time (see [Supported models](/models)). |
