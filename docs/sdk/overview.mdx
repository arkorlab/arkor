...
| [`CloudApiClient`](/sdk/deployments) | `core/client.ts` | Typed client for the cloud API. Exposes `listDeployments` / `createDeployment` / `createDeploymentKey` etc. for managing `*.arkor.app` inference URLs. See [Deployments](/sdk/deployments) for the full method list and examples. |
...
`Arkor`, `ArkorInput`, `ArkorProjectState`, `BlobDatasetSource`, `ChatMessage`, `DatasetSource`, `HuggingfaceDatasetSource`, `InferArgs`, `JobStatus`, `LoraConfig`, `ReasoningEffort`, `ResponseFormat`, `StructuredOutputs`, `ToolCall`, `ToolChoice`, `ToolDefinition`, `Trainer`, `TrainerCallbacks`, `TrainerInput`, `TrainingJob`, `TrainingMetrics`, `TrainingResult`, plus `OAuthCredentials`, `AnonymousCredentials`, and `Credentials` for the auth helpers.

Deployment-related type-only exports (see [Deployments](/sdk/deployments)): `CloudApiClientOptions`, `DeploymentTarget`, `WritableDeploymentTarget`, `DeploymentAuthMode`, `DeploymentRunRetentionMode`, `DeploymentDto`, `DeploymentKeyDto`, `DeploymentScope`, `CreateDeploymentInput`, `UpdateDeploymentInput`, `CreateDeploymentKeyInput`, `CreateDeploymentKeyResult`. The runtime `CloudApiClient` and `CloudApiError` classes are listed under Utilities above; they're value exports, not types, so import them as values.

`TrainingLogContext` and `CheckpointContext` are not exported by name today; they are documented in the [callbacks](/sdk/callbacks) page so you can mirror them inline if you need typed callback parameters.
...
- [Run lifecycle](/concepts/lifecycle) for what `start()` / `wait()` actually do
- [Project structure](/concepts/project-structure) for where the manifest and trainer live in a project
- [CLI overview](/cli/overview) for the surface that drives the SDK from the shell
