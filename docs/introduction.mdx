...
- Run training on Arkor's managed GPUs with no separate infra setup. Try it without an account; run `arkor login --oauth` (`pnpm exec arkor login --oauth` / `npx arkor login --oauth` / `yarn arkor login --oauth` / `bunx arkor login --oauth`) before your next run if you want it tied to an account. Merging anonymous workspaces and jobs into an account once you sign in is on the roadmap.
