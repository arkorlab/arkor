...
On an Apple Silicon Mac, `arkor dev --local` runs the same Studio against a local MLX training server: no account, jobs train on your machine, and the Playground chats through a local inference server. See [Train locally with MLX](/guides/cli/local-training).
