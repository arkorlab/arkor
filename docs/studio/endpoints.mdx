...
When the project state isn't on disk yet (you haven't run `arkor dev` against this directory) the list shows empty and Studio doesn't make an upstream call. There's no scope to send without `(orgSlug, projectSlug)`.
