...
description: "Studio's Overview, Jobs, and Job detail pages: trigger a run, browse the list, inspect a single job."
---

# Run training and watch jobs

The training surface is split across three routes: **Overview** (`#/`) hosts the **Run training** action, **Jobs** (`#/jobs`) is the searchable, filterable, auto-refreshing list of every run, and **Job detail** (`#/jobs/:id`) is where the live event stream and the loss chart live. Clicking a row in the list navigates to its detail page.
...
The Jobs page (`#/jobs`) polls `GET /api/jobs` once at mount, then every 5 seconds. A **Refresh** icon in the card header forces an immediate reload between ticks (the icon spins while the request is in flight).

Two client-side reducers narrow what the table renders:

- **Search** (name or ID, case-insensitive). The text input filters rows where `Job.name` or `Job.id` contains the query.
- **Status filter** chips: `All` / `Running` / `Completed` / `Queued` / `Failed` / `Cancelled`. Exactly one is active at a time.

The empty state distinguishes the two cases: `No jobs yet` (the project genuinely has no runs; the description points you at Overview as plain text, since the trigger lives there) vs `No matches` (the active search / filter hides every row; relax the filter or clear the query).
...
The list shows whatever order the backend returned. There is no client-side pagination; the visible-rows reducer relies on the server returning a complete list.

## Job detail

**Cancel run.** While the job is `queued` or `running`, the header carries a **Cancel run** button. It cancels the job on the backend (the same call [`trainer.cancel()`](/sdk/trainer-control#cancel) makes), so the training process actually stops. This is distinct from the Run-training card's **Stop training** button, which only detaches from the runner's output stream: in local mode the trainer belongs to the `arkor dev` process and keeps running until the job itself is cancelled.
...
- **No cancel once a job is terminal.** The **Cancel run** button only appears while a job is `queued` or `running`; a finished job has nothing left to stop.
