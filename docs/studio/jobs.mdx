...
The empty state distinguishes the two cases: `No jobs yet` (the project genuinely has no runs — links you back to Overview to start one) vs `No matches` (the active search / filter hides every row — relax the filter or clear the query).
