...
| `training.log` | The `step`, `loss`, and (when present) `evalLoss` are appended to the loss chart's data array. The event log row reads `step=<n> loss=<value> evalLoss=<value>` (the event name is rendered in its own column); the `evalLoss=` segment is omitted on steps without one. |
...
Hovering shows the nearest step and whichever of `loss` / `evalLoss` are present at that step (eval-only steps don't show a `loss` value, and vice-versa). If no `training.log` event has arrived yet, the chart slot reads `Waiting for training.log events…`.
