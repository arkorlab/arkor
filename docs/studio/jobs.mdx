...
The Run training panel calls `/api/manifest` once at mount, then 5 seconds after each response. Every call rebuilds the trainer, so a slow rebuild stretches that gap beyond 5 seconds. The response is the project's `createArkor({ trainer })` summary, which the panel uses to label the action:
...
There is no input form for picking the trainer or passing flags: Studio always runs the trainer registered through `createArkor`, and `arkor start` reuses `.arkor/build/index.mjs` if it already exists. Under `arkor dev` that artifact stays current on its own: the manifest poll described above rebuilds on every call, so an edit to `src/arkor/` is picked up on the next poll, with no reload. See [CLI § build / start](/cli/build-and-start) for the precise rebuild rules.
