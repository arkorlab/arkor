...
Once a run has exceeded the 2,000-point cap and the chart has compacted at least once, the training-loss statistics above describe the shape of what's currently retained rather than a strictly unbiased sample of the full run, since compaction favors keeping spikes over ordinary points. Eval-loss statistics are unaffected in the common case, since `evalLoss` points are kept in full unless that series alone exceeds the cap.
