...
| Column | Source |
| --- | --- |
| Status | `Job.status` (`queued` / `running` / `completed` / `failed` / `cancelled`) with one Studio-derived display state: a `queued` job whose stream has connected (or whose `createdAt` is within the last 90 s) renders as `Warming up GPU`. The cell carries a CSS class for colouring. |
