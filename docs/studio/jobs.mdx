...
The Run training panel calls `/api/manifest` once at mount, then every 5 seconds. The response is the project's `createArkor({ trainer })` summary, which the panel uses to label the action:
