...
The Run training panel calls `/api/manifest` when the page loads, then again 5 seconds after each response for as long as the page is open. The response is the project's `createArkor({ trainer })` summary, which the panel uses to label the action:
