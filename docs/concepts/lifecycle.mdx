...
Fires once, on success. `artifacts` lists what the backend produced for this run, and `metrics` is the run-level summary (`finalLoss`, `totalSteps`, `totalTime`); it is always an object, with `null` on any field the backend did not report. Use it to:

- Persist the final adapter ID where the rest of your app can find it.
- Record the final loss without re-deriving it from the last `onLog` frame.
...
Fires once if the backend reports a failure. Note that `error` is a `string` (the message the backend sent), not an `Error` instance. `step` is the training step the run died on, or `null` when it failed before the training loop started:

```ts
onFailed: ({ job, error, step }) => {
  console.error(`Run ${job.id} failed at step ${step}: ${error}`);
},
```

`onFailed` is for backend-reported failures only. A thrown exception inside one of your own callbacks does **not** route through `onFailed`; it rejects `wait()` immediately with your error. A thrown callback is **not** treated as an SSE transport failure, so it is not routed through the reconnect loop or retried: the reconnect loop handles only transient stream failures (a dropped connection, a retryable HTTP error such as `404`/`408`/`429`/`5xx` on open; permanent statuses like `401`/`403`/`410`/`426` reject immediately, see [Trainer control § Reconnects](/sdk/trainer-control#reconnects)). This is deliberate, because the failing event's `Last-Event-ID` has already advanced, so retrying would skip the event, swallow your error, and (for a terminal `training.completed` event) resolve `wait()` with empty artifacts. If you need non-fatal handling, catch errors inside the callback and decide what to do (log, abort, persist) before they escape.
