...
- `arkor dev` starts the Studio web server at `127.0.0.1:4000` by default and writes `~/.arkor/studio-token`. Without an explicit `-p`, a port already in use falls back to the next free one within a bounded range, so open the URL the CLI prints. It does not file-watch `src/arkor/`. Studio's `/api/manifest` endpoint calls `runBuild` and re-imports through a content-addressed copy of the bundle (so unedited rebuilds reuse Node's module cache and edits bust it), and the Run training page polls it every few seconds, so edits to `src/arkor/` are picked up without a page reload. `arkor dev` does not write `.arkor/state.json` on its own.
