...
├── arkor.config.ts
├── AGENTS.md           # AI-agent rules (skip with create-arkor's --no-agents-md)
...
and its `@<path>` directive is a built-in import — the single line
`@AGENTS.md` inlines the AGENTS.md contents into Claude's context, so
both files stay in sync without duplication.
