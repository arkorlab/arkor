...
| `model`   | `SupportedModel` | Narrowed to the models the backend accepts today; an unsupported value fails to compile and makes `createTrainer` throw. See [Supported models](/models). |
