---
title: "Studio"
description: "The local web UI for starting training runs, watching them stream, chatting with finished models, and publishing them at *.arkor.app URLs."
---
...
A note on the dev loop: Studio's `/api/manifest` endpoint rebuilds your trainer on every request and imports the result through a content-addressed copy of the bundle (`index.<hash>.mjs`, see `packages/arkor/src/studio/manifest.ts`), so an unedited rebuild reuses Node's module cache while an edit produces a fresh module. The Run training page polls that endpoint every few seconds, so edits to `src/arkor/` are picked up without a page reload, and a trainer you register mid-session enables the Run training button on its own.

## Where Studio runs

When you start `arkor dev`, the CLI:

1. Boots a Hono server on `127.0.0.1:4000` by default (use `-p` to change the port). With no `-p`, a busy port falls back to the next free one within a bounded range, so the URL the CLI prints is the one to open; see [`arkor dev`](/cli/dev) for the retry rules.
2. Serves a Vite + React SPA from the same origin so the UI talks to the CLI through `/api/*` on loopback.
3. Issues a per-launch CSRF token (saved to `~/.arkor/studio-token` with mode `0600`) and requires every `/api/*` request to present it. The SPA itself carries no token check; that is how the page receives the token in the first place.

The server only binds to loopback and rejects requests with a non-loopback `Host` header. There is no public URL and no remote access path, and the token rotates every time you start `arkor dev`.
...
Studio is a development tool. It runs on your machine, only on loopback, and only while `arkor dev` is up. For production usage of a fine-tuned model, publish it as a [deployment](/sdk/deployments) and call the endpoint's OpenAI-compatible API from your application. `infer` is only handed to `onCheckpoint` during a run, and Studio is never something to point users at.
