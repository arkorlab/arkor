...
- **Different hash (model / dataset / hyperparameters changed).** The runner is signalled with SIGTERM; the trainer's internal early-stop entry point waits for the next checkpoint upload to finish before issuing `cancel()`, then the SPA re-spawns the run with the rebuilt artifact. The previous Cloud-side job reaches `cancelled` after the checkpoint is uploaded, so the partial work is preserved as an artifact. One caveat: if no checkpoint arrives within the early-stop window (about five minutes), the runner falls back to cancelling immediately, so a run that checkpoints rarely can lose the work since its last checkpoint.

Stopping a run early from your own code (rather than from the dev loop) is built on the public [`abortSignal` + `cancel()`](/sdk/trainer-control#abortsignal) pair; the [Early stopping recipe](/cookbook/early-stopping) walks through it. Note that the pair stops the run *now*: `abortSignal` stops your local waiting and `cancel()` asks the backend to stop the job. If you also want the "wait for the next checkpoint first" behaviour the dev loop gets, combine them with an [`onCheckpoint`](/concepts/lifecycle) callback and trigger the abort + cancel from inside the next checkpoint event instead of immediately.
