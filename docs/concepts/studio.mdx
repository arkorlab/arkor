...
3. Issues a per-launch CSRF token (saved to `~/.arkor/studio-token` with mode `0600`) and requires every `/api/*` request to present it. The SPA itself is served unauthenticated; that is how the page gets the token in the first place.
