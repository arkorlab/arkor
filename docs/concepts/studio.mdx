...
If `~/.arkor/credentials.json` is missing, the entry point decides what to do. **`arkor dev`** always bootstraps an anonymous session at launch (regardless of whether the deployment offers OAuth) and prints a one-line hint pointing at `arkor login --oauth` so you can upgrade to a real account whenever you want; it never auto-launches the OAuth flow. The **Studio server's lazy bootstrap** (when an `/api/*` request arrives before credentials are on disk) does the same anonymous fallback. To use an account session on an OAuth-configured deployment, run `arkor login --oauth` separately before (or after) clicking around in Studio; the credentials file is shared, so Studio picks up the account session on its next request.
