...
The server only binds to loopback and rejects requests with a non-loopback `Host` header. There is no public URL and no remote access path (local requests still need the token), and the token rotates every time you start `arkor dev`.
