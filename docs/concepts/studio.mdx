...
1. Boots a Hono server on `127.0.0.1:4000` by default (use `-p` to change the port). With no `-p`, a busy port falls back to the next free one, so the URL the CLI prints is the one to open; see [`arkor dev`](/cli/dev) for the retry rules.
