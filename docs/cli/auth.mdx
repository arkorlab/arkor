...
Three short commands that read or write `~/.arkor/credentials.json`. None of them touches `.arkor/state.json`. For anonymous workspaces, project routing is auto-created by the runtime on the first call that needs a scope: `trainer.start()`, base-model inference, or creating the first `*.arkor.app` deployment from Studio's [Endpoints](/studio/endpoints) page. For OAuth workspaces, `.arkor/state.json` has to exist before any of those calls: today, neither `arkor login` nor `arkor init` creates it, so the practical path is to create it manually with `{ orgSlug, projectSlug, projectId }`. The runtime points you at this fallback when it errors out on a missing state file.
...
Deletes `~/.arkor/credentials.json` after confirmation. The confirmation
defaults to **No**.

For anonymous credentials, logout is irreversible unless you backed up the
credentials file yourself: deleting it removes the local token for that
anonymous identity, and the next anonymous login creates a new `anonymousId`.
OAuth credentials can be recreated by running `arkor login --oauth` again.

`arkor logout` does not touch the project-local `.arkor/state.json`. If you
were anonymous and start a fresh anonymous session in the same workspace, that
leftover file points at the previous identity's organization: read views ignore
it (lists stay empty), but the first write (training, base-model inference,
endpoint creation) fails with a 409 / error asking you to delete
`.arkor/state.json` to start a fresh anonymous project, or `arkor login` with
the account that owns it. See [Project structure](/concepts/project-structure).
...
| Flag | Description |
| --- | --- |
| `-f, --force` | Delete credentials without prompting. |

### Behavior

- If the credentials file does not exist, the CLI logs `No credentials on file.` and exits.
- If the credentials file is anonymous (`mode: "anon"`), the CLI warns that the anonymous identity cannot be restored after logout.
- If the user declines the prompt, the CLI logs `Aborted.` and exits without deleting.
- `--force` skips the prompt, but still prints the anonymous restore warning before deleting anonymous credentials.
...
Both modes write to the same file at `~/.arkor/credentials.json` and are tagged with a `mode` field of either `"oauth"` or `"anon"` so the rest of the CLI (and the Studio server) knows which path to use. See [Project structure](/concepts/project-structure) for the full layout.

## Token expiry

For OAuth sessions, the credentials file records both the access token and the issued refresh token, plus the `expiresAt` timestamp returned by the token exchange. The refresh token is stored today, but the CLI does **not** yet auto-refresh expired access tokens; that path is [on the roadmap](/roadmap#oauth-token-auto-refresh).
...
| `Failed to fetch /v1/me (<status>). Token may be expired.` | `arkor whoami` | The cloud-api rejected the request with a non-200, non-426 status. Most often expired access tokens. | Re-run `arkor login` matching the current `mode`: `arkor login --oauth` for `mode: "oauth"`, `arkor login --anonymous` for `mode: "anon"` (the latter mints a new `anonymousId`, so it lands in a new workspace). The exit code stays `0` so wrapper scripts can inspect the message. |
...
- [Roadmap § OAuth token auto-refresh](/roadmap#oauth-token-auto-refresh)
