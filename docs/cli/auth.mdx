...
Sign in to the managed backend. With no flags the CLI checks the deployment's `cli/config` and, when OAuth is available, asks interactively whether to sign in via OAuth or as an anonymous session. The interactive selector defaults to **Anonymous**, so accepting the default (or piping into a non-TTY) lands on the anonymous path; OAuth runs only when you explicitly select it or pass `--oauth`. When OAuth is not configured for the deployment, the CLI prints `OAuth is not configured — continuing with an anonymous session.` and skips the prompt entirely.
...
| Flag | Description |
| --- | --- |
| `--oauth` | Skip the picker and run the Auth0 Authorization Code + PKCE flow directly. Mutually exclusive with `--anonymous`. Errors out with `OAuth is not configured for this deployment. Re-run without --oauth or pass --anonymous.` if the deployment has no Auth0 settings, and with `--oauth needs a browser callback that CI runners can't complete. Use --anonymous in CI.` when `process.env.CI` is set (PKCE needs a browser callback, so the loopback would hang forever in CI). |
| `--anonymous` | Skip the picker and request a throwaway anonymous token. Mutually exclusive with `--oauth`. This is also what `arkor dev` does at launch when no credentials exist on disk: it always bootstraps anonymous first, regardless of whether the deployment offers OAuth. |
...
2. If `--anonymous` is passed, or OAuth is not configured for the deployment, it requests an anonymous token from `/v1/auth/anonymous` and writes it to `~/.arkor/credentials.json` with `mode: "anon"`.
3. If OAuth is configured and `--oauth` is passed, it skips straight to the OAuth flow.
4. If OAuth is configured and no flag is passed, it shows an interactive picker (`OAuth (browser)` / `Anonymous`) with **Anonymous** preselected. Accepting the default — including in non-interactive contexts where the prompt is non-blocking — runs the anonymous path; choosing OAuth runs the OAuth flow.
...
Anonymous credentials let you try Arkor without an account: training runs, jobs, and any work you do are tied to the local machine via the anonymous token. Switching to OAuth later (`arkor login --oauth`, or selecting `OAuth (browser)` from the picker) replaces the credentials file but does not migrate the work; if you want to keep what you trained anonymously, run `arkor login --oauth` before you start the run.
...
- The fix is to re-run `arkor login --oauth`, which goes through the full PKCE flow again and overwrites `~/.arkor/credentials.json` with fresh tokens.
...
| `OAuth is not configured for this deployment. Re-run without --oauth or pass --anonymous.` | `arkor login --oauth` | The deployment's `cli/config` has no Auth0 settings, so PKCE has nowhere to go. | Re-run without `--oauth` (the CLI will fall through to anonymous), or pass `--anonymous` explicitly. |
...
| `Auth0 did not return a refresh token. Make sure the Application has 'offline_access' scope enabled.` | `arkor login --oauth` | The Auth0 token endpoint succeeded but the response had no `refresh_token`. Usually a deployment misconfiguration on the Auth0 side. | Enable the `offline_access` scope on the Auth0 Application, or report the deployment to your Arkor admin. |
...
| `Failed to fetch /v1/me (<status>). Token may be expired.` | `arkor whoami` | The cloud-api rejected the request with a non-200, non-426 status. Most often expired access tokens. | Run `arkor login --oauth` again. The exit code stays `0` so wrapper scripts can inspect the message. |
