...
| --- | --- | --- |
| `--local` | off | Train on this machine via the separately installed `@arkor/local` runtime (MLX on Apple Silicon) instead of Arkor Cloud. A local training server boots in-process, the trainer streams the same events it would from the cloud, and the finished adapter lands under `.arkor/local/`. See [Local training](/cli/local-training). |
