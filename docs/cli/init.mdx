...
Every scaffold writes a small `pnpm-workspace.yaml` alongside `package.json`:
...
— or flip the entry to `true` in `pnpm-workspace.yaml` after the fact. We accept the flag regardless of `--use-<pm>` so a project scaffolded with `--use-npm` today and switched to pnpm later doesn't silently strip the earlier `--allow-builds` choice.
