...
| `--use-bun` | Force bun as the package manager. (Known issue on Windows: `arkor init --use-bun` produces an installed `node_modules` but no `bun.lock`, so the bootstrap commit's lockfile-in-HEAD invariant doesn't hold. The CI install matrix gates Windows × bun out of the `arkor init` lane for this reason — see `.github/workflows/ci.yaml`'s `install-matrix` comment. `bun install` directly, and `create-arkor --use-bun`, both work normally on Windows; only the `arkor init` spawn shape exhibits the divergence.) |
...
The scaffolder touches up to ten paths: six unconditional plus two AGENTS.md-related plus two conditional package-manager configs. None of them overwrites existing user content:
