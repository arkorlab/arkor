...
| [`arkor dev`](/cli/dev) | Launch [Studio](/concepts/studio) on `http://localhost:4000` (configurable). Pass `--local` to run against a [local training server](/cli/local-training) instead of Arkor Cloud. |
| [`arkor build`](/cli/build-and-start) | Bundle `src/arkor/index.ts` to `.arkor/build/index.mjs`. |
| [`arkor start`](/cli/build-and-start) | Run the build artifact (auto-builds when missing). Pass `--local` to [train on this machine](/cli/local-training) via MLX. |
...
| [`arkor logout`](/cli/auth) | Delete `~/.arkor/credentials.json` after confirmation; anonymous identities cannot be restored after deletion. |
...
| `ARKOR_LOCAL_SERVER_URL` / `ARKOR_LOCAL_SERVER_TOKEN` | The hand-off between `arkor start --local` / `arkor dev --local` and the trainer: URL and per-launch bearer token of the in-process [local training server](/cli/local-training). Managed by the CLI (set in-process and injected into Studio's Run-training children); not meant to be set by hand, and setting only one of the pair is an error. |
