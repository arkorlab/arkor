...
| `ARKOR_TELEMETRY_DEBUG=1` | Log the events that would be sent to PostHog instead of sending them. Useful for verifying which command surfaces emit which events. |
| `CI=1` (or any non-TTY stdout) | Force non-interactive mode. `arkor init` skips its prompts and uses the values you passed via flags (or built-in defaults under `--yes`); the git-init prompt is treated as "no" unless you pass `--git` explicitly. See [`arkor init` § In CI / non-interactive shells](/cli/init#in-ci---non-interactive-shells). |

### On the roadmap

Pointing the CLI at a non-default cloud-api endpoint (for self-hosted or staging deployments) is not yet a stable user-facing knob and should be treated as internal until announced. The cloud-api URL is currently hard-pinned to the Arkor-managed default.
