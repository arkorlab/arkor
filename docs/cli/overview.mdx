...
| [`arkor login`](/cli/auth) | Sign in. With no flags, prompts interactively (default selection: `Anonymous`). Pass `--oauth` for the loopback Auth0 PKCE flow or `--anonymous` to skip the picker. |
