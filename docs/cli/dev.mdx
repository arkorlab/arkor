...
| `403 — Studio API is loopback-only` (in browser devtools) | The `Host` header is something other than `127.0.0.1` / `localhost`. | Reach Studio via `http://localhost:<port>` or `http://127.0.0.1:<port>`. Reverse proxies or `0.0.0.0`-bound shells will be rejected by design. |
| `403 — Missing or invalid studio token` (in browser devtools) | The CSRF token in the page does not match the current launch. Usually a stale tab from a previous `arkor dev`. | Reload the tab. Token rotates on every launch. |
