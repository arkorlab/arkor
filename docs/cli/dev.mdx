...
- **No `--port` (using the `4000` default).** If `4000` is taken (another `arkor dev` left running, an unrelated dev server, etc.), `arkor dev` silently retries on `4001`, `4002`, and so on, up to 10 attempts, and prints the port it actually bound: ``Arkor Studio running on http://localhost:<port>``. If every port in that range is busy, it exits non-zero with ``Port <requested> is already in use, and no free port was found in <requested>-<last>. Pass --port to choose one explicitly.``
