...
Then, from the agent's shell, read the token out of the session file and confirm the server. Prefer the exact path printed on stdout. If you must glob, select the **newest** file (`ls -t`), never the lexically first: a crashed prior session can leave an older `session-*.json` behind, and picking it would hand you a dead token. (`arkor dev --agent` also reaps dead-pid session files on startup, so stale files are cleared on the next launch.)
