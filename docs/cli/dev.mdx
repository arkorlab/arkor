...
| `--local` | off | Run Studio against a local training server (the separately installed `@arkor/local` runtime, MLX on Apple Silicon) instead of Arkor Cloud. No credentials are bootstrapped, jobs train on this machine, and the Playground chats through a local inference server. See [Local training](/cli/local-training). |
| `--backend <id>` | auto-detect | Pick a specific local backend (today: `mlx`). Implies `--local`. |
...
<Note>
  The sequence below describes a cloud-backed launch. With `--local`, step 1 is skipped entirely (local mode never reads, mints, or persists credentials); steps 2 to 4 are unchanged, and the local training server is started before the listener binds so a preflight failure never leaves a half-open Studio.
</Note>
