<p align="center">
  <img src="assets/oddphin-rounded.png" alt="Arkor のマスコット Oddphin" width="96">
</p>
...
3 つすべてが同じ小さなオープンウェイトベースモデル (`gemma-4-E4B-it`) と HuggingFace 上の厳選されたパブリックデータセットを組み合わせており、最初の実行は数分で完了します。
...
私たちが欲しかった、しかし見つからなかったのは、TypeScript と Node の開発者がすでに行っている働き方に馴染む道筋でした。ファインチューニング、評価、推論サービスがプロダクトと同じコードベースに同居し、同じエディター、型、レビューフローで扱えるようなワークフローです。
...
- [x] **自分のマシーンでトレーニングもチャットも。** `arkor start --local` / `arkor dev --local` は、オプトインの [`@arkor/local`](https://www.npmjs.com/package/@arkor/local) パッケージを通じて、同じトレーナーを Apple Silicon 上のローカル MLX バックエンドで実行します (チェックポイント推論も含む)。詳しくは[ローカルトレーニングガイド](https://docs.arkor.ai/ja/cli/local-training)へ。
...
| `arkor start`                                      | ビルド成果物を実行 (自動ビルド。`--local` でこのマシーン上で学習)       |
