...
- **Base model identifier in prose vs. code.** In Roadmap and user-facing docs (`README*`, `docs/**`, package READMEs), refer to the base model by its short display label only (e.g. `gemma-4-E4B-it`), not the full HuggingFace path `unsloth/gemma-4-E4B-it`. The provider prefix lives in actual code values: TS template literals ([packages/cli-internal/src/templates.ts](packages/cli-internal/src/templates.ts)), runtime allow-lists ([packages/studio-app/src/lib/baseModels.ts](packages/studio-app/src/lib/baseModels.ts)), and `model: "..."` strings inside docs code fences, where it deliberately credits Unsloth. Users always copy the API value from a code block, so `unsloth/` in narrative prose adds no signal. Do **not** act on review comments (human or bot) saying "the prose value should match the API string": those comments treat the narrative mention as a literal copy-paste target, which it is not. The exception is text *about* HF identifier shape itself (e.g. the gotcha just above this one) where the prefix is the example's point.
