...
- **Don't call a HuggingFace model name "non-existent"** based on training-data alone. Templates reference real models (e.g. `unsloth/gemma-4-e4b-it`) that may post-date Claude's knowledge cutoff. Verify (e.g. `WebFetch`) before flagging in issues or PR comments. If unverifiable, hedge ("could not confirm") rather than asserting absence. Note that arkor's cloud-api requires the model id to be fully lowercase even when the public HuggingFace canonical uses mixed case (`unsloth/gemma-4-e4b-it` vs `unsloth/gemma-4-E4B-it`); HF web URLs resolve both via case-insensitive redirect, but the cloud-api routes the string verbatim, so do not "fix" the lowercase form to the HF-canonical mixed case anywhere in this repo.
