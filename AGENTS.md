...
pnpm lint           # oxlint --deny-warnings across all packages (single .oxlintrc.json at the root)
...
- **Single root `.oxlintrc.json`**: lint config lives once at the repo root ([`.oxlintrc.json`](.oxlintrc.json)) with `overrides` for the studio-app SPA (React/jsx-a11y plugins, browser env), test/spec files (vitest plugin), and the Playwright fixture path (`eslint/no-empty-pattern` off, because Playwright's `({}, use) => ...` fixture signature requires an empty destructure). Each workspace's `lint` script is just `oxlint --deny-warnings .`; oxlint walks up the tree to find the root config. Add new overrides at the root rather than introducing per-package `.oxlintrc.json` files.
