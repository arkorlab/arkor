...
- **oxlint native bindings + `minimumReleaseAge`**: pnpm 10.x has a known interaction where `minimumReleaseAge` can cause some `optionalDependencies` (notably the host platform's native binding) to be skipped during a fresh `pnpm add`, leaving `oxlint --version` to die with "Cannot find native binding" even though the gate's 24h window has passed. The workaround when bumping oxlint is `pnpm add -Dw oxlint@<version> --config.minimum-release-age=0`; once the binding entries land in `pnpm-lock.yaml`, regular `pnpm install` (and CI's `--frozen-lockfile`) resolves them normally. Do not paper over this by adding an explicit `@oxlint/binding-<platform>` devDependency: that would pin a platform-specific binding into the root `package.json` and break installs on other machines.
